...
class MsdsTest(TestCase):
    ...
    def test_add_msds(self):
        ...
        response = self.client.post(reverse('msds:msds_msdsobject_create', kwargs=self.url_attr), data=data)
    ...
    def test_update_msds(self):
        ...
        response = self.client.post(reverse('msds:msds_msdsobject_update', kwargs=self.url_attr), data=data)
        pos_count= MSDSObject.objects.count()
        self.assertEqual(response.status_code, 302)
        self.assertTrue(pos_count==pre_count)
    ...
    def test_delete_msds(self):
        ...
        response = self.client.post(reverse('msds:msds_msdsobject_delete', kwargs=self.url_attr))
        pos_count= MSDSObject.objects.count()
        self.assertEqual(response.status_code, 302)
        self.assertTrue(pos_count<pre_count)
    ...
    def test_detail_msds(self):
        ...
        response = self.client.get(reverse('msds:msds_msdsobject_detail', kwargs=self.url_attr))
        self.assertEqual(response.status_code, 200)
