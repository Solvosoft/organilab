...
def build_qr_instance(url, object, organization):
    ...
    return QRModel.objects.create(
        qr_url=url,
        organization_id=organization,
        b64_image=b64_image,
        qr_image=img_svg.decode(),
...
def update_qr_instance(url, object, organization):
    ...
    img_svg = file.getvalue()
    b64_image = base64.b64encode(img_svg).decode()
    QRModel.objects.filter(**cc_filters).update(
        qr_url=url, b64_image=b64_image, qr_image=img_svg.decode()
    )
...
def get_qr_by_instance(object, organization):
    ...
    return QRModel.objects.filter(
        content_type=ct, organization=organization, object_id=object.pk
    ).last()
