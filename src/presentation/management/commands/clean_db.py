...
class Command(BaseCommand):
    ...
    def delete_organizations(self):
        ...
        OrganizationStructure.objects.annotate(lab_count=Count("laboratory")).filter(
            lab_count=0
        ).delete()

        OrganizationStructure.objects.annotate(
            lab_count=Count("userorganization")
        ).filter(lab_count=0).delete()

    def delete_users(self):
        User.objects.annotate(user_count=Count("userorganization")).filter(
            user_count=0
        ).delete()

    def delete_laboratory(self):
        labs = Laboratory.objects.annotate(
            shobj_count=Count("laboratoryroom__furniture__shelf__shelfobject")
        ).filter(shobj_count=0)
        # .delete()
