...
from laboratory.models import Object, SustanceCharacteristics, OrganizationStructure, ObjectFeatures, Catalog
    ...
    def handle(self, *args, **options):
        for files in options['json_file']:
            ...
            for instance in elements:
                ...
                instance['fields']['security_sheet'] = self.get_sheet(instance['fields'])
                if 'security_sheet_name' in instance['fields']:
                    del instance['fields']['security_sheet_name']

                instance['fields']['iarc'] = self.get_catalog(instance['fields']['iarc'], True)
                instance['fields']['imdg'] = self.get_catalog(instance['fields']['imdg'], True)
                instance['fields']['precursor_type'] = self.get_catalog(instance['fields']['precursor_type'], True)

                white_organ = self.get_catalog(instance['fields']['white_organ'], False)  # M
                ue_code = self.get_catalog(instance['fields']['ue_code'], False)
                nfpa = self.get_catalog(instance['fields']['nfpa'], False)
                storage_class = self.get_catalog(instance['fields']['storage_class'], False)
