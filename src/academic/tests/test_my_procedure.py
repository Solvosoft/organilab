...
class MyProceduresViewTest(TestCase):
    ...
    def test_add_my_procedures_forbidden_for_non_organization_member_users(self):
        self.client.force_login(self.external_user)
        url = reverse("academic:add_my_procedures",
                      kwargs={"content_type": 'laboratory',
                              "model": 'laboratory',
                              "lab_pk": self.lab.pk,
    ...
    def test_remove_my_procedures_forbidden_for_non_organization_member_users(self):
        ...
        self.client.force_login(self.external_user)
        url = reverse("academic:remove_my_procedure",
    ...
    def test_update_my_procedure(self):
        ...
        url = reverse("academic:complete_my_procedure",
                      kwargs={"org_pk": self.organization.pk,
                              "lab_pk": self.lab.pk,
                              "pk": my_procedure.pk})
    ...
    def test_create_my_procedure(self):
        ...
        success_url = reverse("academic:get_my_procedures",
                              kwargs={"org_pk": self.organization.pk,
                                      "lab_pk": self.lab.pk})
        self.assertRedirects(response, success_url)
        self.assertIn("Second my procedure", list(MyProcedure.objects.values_list("name", flat=True)))
    ...
    def test_delete_my_procedure(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertRedirects(response, success_url)
        self.assertNotIn("First my procedure test", list(MyProcedure.objects.values_list("name", flat=True)))
...
class CompleteMyProcedureViewTest(TestCase):
    ...
    def test_update_comment(self):
        step = ProcedureStep.objects.get(pk=17)
        my_procedure = MyProcedure.objects.get(pk=1)
        comment = CommentProcedureStep.objects.filter(procedure_step=step, my_procedure=my_procedure).first()
        data = {
    ...
    def test_delete_comment(self):
        step = ProcedureStep.objects.get(pk=17)
        my_procedure = MyProcedure.objects.get(pk=1)
        comments_list = CommentProcedureStep.objects.filter(procedure_step=step, my_procedure=my_procedure)
