...
class ProcedureStepCommentSerializer(serializers.ModelSerializer):
    ...
    created_by_at = serializers.DateTimeField(
        required=False, format=formats.get_format("DATETIME_INPUT_FORMATS")[0]
    )
...
class ProcedureSerializer(serializers.ModelSerializer):
    ...
    def get_actions(self, obj):
        ...
        action = ""
        action += (
            """<a href="%s" title="%s"><i class="fa fa-eye  fa-sm p-2"></i></a>"""
            % (
                reverse("academic:procedure_detail", kwargs=procedure_kwargs),
                _("View Steps"),
            )
        )
        action += (
