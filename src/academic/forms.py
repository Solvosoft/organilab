...
class MyProcedureForm(forms.ModelForm, GTForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            "custom_procedure": AutocompleteSelect(
...
class StepForm(GTForm, forms.Form):
    ...
    description = forms.CharField(widget=TextareaWysiwyg, label=_("Description"))
...
class ReservationForm(GTForm, forms.Form):
    ...
    def clean(self):
        ...
        if initial_date and final_date:
            ...
            if difference < 0:
                self.add_error(
                    "final_date", _("Final date can't be lower than initial date")
                )


class ValidateProcedureReservationForm(ReservationForm):
    procedure = forms.ModelChoiceField(
        widget=genwidgets.Select(), queryset=Procedure.objects.all(), required=True
    )
