...
class DerbAPITest(TestCase):
    ...
    def create_object_features(self):
        return ObjectFeatures.objects.create(
            name="Test Feature", description="Test Feature Desc"
        )
    ...
    def create_object(self):
        ...
        object = Object.objects.create(
            code="1234", name="Test Object", organization=org
        )
    ...
    def test_lab_api_by_user(self):
        response = self.client.get(
            reverse("derb:api_laboratory_by_user", kwargs=self.url_attr)
        )
    ...
    def test_lab_api_by_org(self):
        response = self.client.get(
            reverse("derb:api_laboratory_by_org", kwargs=self.url_attr)
        )
        self.assertEqual(response.status_code, 200)
        self.assertContains(response=response, text="Vick Lab")
        self.assertContains(response=response, text="Lab Ultra")
    ...
    def test_inform_api(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertContains(
            response=response, text="Uso de instrumentos de laboratorio"
        )
    ...
    def test_users_by_org_api(self):
        response = self.client.get(
            reverse("derb:api_org_structure", kwargs=self.url_attr)
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_objects_api(self):
        self.create_object()
        response = self.client.get(
            reverse("derb:api_objects", kwargs=self.url_attr) + "?limit=100"
        )
        self.assertEqual(response.status_code, 200)
