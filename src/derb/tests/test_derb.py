...
class DerbTest(TestCase):
    ...
    def test_create_form(self):
        ...
        data = {
            ...
            "status": "admin",
            "schema": {
                "name": "Prueba Congreso",
                "status": "admin",
                "components": []
            }
        }
        response = self.client.post(reverse('derb:create_form', kwargs=self.url_attr),data=data)
        pos = CustomForm.objects.count()
        self.assertEqual(response.status_code, 200)
        self.assertTrue(pre<pos)
        self.assertTrue(data['name']==CustomForm.objects.last().name)
        self.assertTrue(json.loads(response.content)['url']== reverse('derb:edit_view', args=[1,CustomForm.objects.last().pk]))
