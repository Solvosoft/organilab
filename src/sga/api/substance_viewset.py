...
class SubstanceViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        queryset = super().get_queryset().filter(organization=self.organization).annotate(cas_id=F('substancecharacteristics__cas_id_number'))
        return queryset
    ...
    def destroy(self, request, org_pk, *args, **kwargs):
        ...
        return JsonResponse(data={'detail': _("Substance deleted successfully")}, status=status.HTTP_202_ACCEPTED)

    def perform_destroy(self, instance):
        organilab_logentry(self.request.user, instance, DELETION, 'SGA Substance '+str(instance), relobj=[self.organization])
        instance.delete()