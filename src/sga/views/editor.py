...
from sga.api.serializers import (
    SGAComplementSerializer,
    BuilderInformationSerializer,
    RecipientSizeSerializer,
)
from sga.forms import (
    ...
    LabelForm,
    PersonalTemplateForm,
    SGALabelForm,
    SGALabelComplementsForm,
    SGALabelBuilderInformationForm,
    PersonalSGAAddForm,
    CompanyForm,
)
...
def template_editor(request, org_pk):
    ...
    instance = tuple(
        filter(
            ...
            (
                request.POST["instance"] if "instance" in request.POST else None,
                request.GET["instance"] if "instance" in request.GET else None,
            ),
        )
    )
    if instance:
        finstance = get_object_or_404(TemplateSGA, pk=instance[0])
...
@login_required
@permission_required(("sga.add_displaylabel", "auth_and_perms.una_can_access"))
def create_personal_template(request, org_pk):
    ...
    context = {
        ...
        "sga_templates": sga_templates,
        "form": PersonalTemplateForm(user=request.user),
        "org_pk": org_pk,
    }
    ...
    if request.method == "POST":
        ...
        if (
            form.is_valid()
            and builder_information_form.is_valid()
            and label_form.is_valid()
        ):
...
def edit_personal_template(request, org_pk, pk):
    ...
    if request.method == "POST":
        ...
        builder_information_form = BuilderInformationForm(
...
@login_required
@permission_required(("sga.delete_displaylabel", "auth_and_perms.una_can_access"))
...
def create_recipient(request, org_pk):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return redirect("sga:add_recipient_size")
    return render(
        request, "add_recipient_size.html", context={"form": form, "org_pk": org_pk}
    )
...
def create_sgalabel(request, org_pk):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            instance.json_representation = form.cleaned_data[
                "template"
            ].json_representation
            ...
            return redirect(
                reverse(
                    "sga:sgalabel_step_one",
                    kwargs={"pk": instance.pk, "org_pk": org_pk},
                )
            )
        else:
...
def sgalabel_step_one(request, org_pk, pk):
    ...
    complement = None
    if sustance:
        complement = SGAComplement.objects.filter(
            substance=sgalabel.label.substance
        ).first()

    complementsga_form = SGALabelComplementsForm(instance=complement, org_pk=org_pk)
    sgabuilderinfo_form = SGALabelBuilderInformationForm(
        user=request.user,
        instance=builderinformation,
    ...
    if request.method == "POST":
        complementsga_form = SGALabelComplementsForm(
            request.POST, instance=complement, org_pk=org_pk
        )
        sgabuilderinfo_form = SGALabelBuilderInformationForm(
            request.POST, user=request.user, instance=builderinformation
        )
        personal_form = PersonalSGAAddForm(
            request.POST, request.FILES, instance=sgalabel
        )
        ...
        if complementsga_form_ok and sgabuilderinfo_form_ok and personal_form_ok:
            return redirect(
                reverse(
                    ...
                    kwargs={"pk": sgalabel.pk, "org_pk": org_pk},
                )
            )
...
@login_required
@permission_required(("sga.view_builderinformation", "auth_and_perms.una_can_access"))
def get_companies(request, org_pk):
    organization = get_object_or_404(
        OrganizationStructure.objects.using(settings.READONLY_DATABASE), pk=org_pk
    )
    user_is_allowed_on_organization(request.user, organization)
    company = BuilderInformation.objects.filter(
        user=request.user, organization=organization
    )
    return render(
        request,
        "company/list_company.html",
        context={"companies": company, "org_pk": org_pk},
    )


@login_required
@permission_required(("sga.add_builderinformation", "auth_and_perms.una_can_access"))
def create_company(request, org_pk):
    ...
    return render(request, "company/add_company.html", context=context)


@login_required
@permission_required(("sga.change_builderinformation", "auth_and_perms.una_can_access"))
...
@login_required
@permission_required(("sga.delete_builderinformation", "auth_and_perms.una_can_access"))
def remove_company(request, org_pk, pk):
    if pk:
        company = BuilderInformation.objects.get(pk=pk)
        company.delete()
        return JsonResponse(
            {"msg": _("The company is removed successfully"), "status": True}
        )
