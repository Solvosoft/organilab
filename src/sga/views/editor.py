...
from sga.api.serializers import SGAComplementSerializer, BuilderInformationSerializer, \
    RecipientSizeSerializer
from sga.forms import SGAEditorForm, EditorForm, PersonalForm, RecipientSizeForm, \
    PersonalSGAForm, \
...
def template_editor(request, org_pk):
    ...
    finstance=None
    instance = tuple(filter(lambda x: x is not None, (request.POST['instance'] if 'instance' in request.POST else None,  request.GET['instance'] if 'instance' in request.GET else None)))
    if instance:
    ...
    if request.method == "POST":
        ...
        if sgaform.is_valid():
            ...
            organilab_logentry(request.user, instance, ADDITION,
                               'SGA Template from editor', relobj=[organization])
...
def create_recipient(request, org_pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect('sga:add_recipient_size')
    return render(request, 'add_recipient_size.html',
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            instance.json_representation = form.cleaned_data[
                'template'].json_representation
            label = Label.objects.create()
            instance.label = label
            instance.save()
            return redirect(reverse("sga:sgalabel_step_one",
...
def sgalabel_step_one(request, org_pk, pk):
    ...
    if sustance:
        complement = SGAComplement.objects.filter(
            substance=sgalabel.label.substance).first()

    complementsga_form = SGALabelComplementsForm(instance=complement, org_pk=org_pk)
    sgabuilderinfo_form = SGALabelBuilderInformationForm(user=request.user,
                                                         instance=builderinformation,
                                                         initial={
                                                             'company': builderinformation.pk if builderinformation else None})
    ...
    if request.method == "POST":
        complementsga_form = SGALabelComplementsForm(request.POST, instance=complement,
                                                     org_pk=org_pk)
        sgabuilderinfo_form = SGALabelBuilderInformationForm(request.POST,
                                                             user=request.user,
                                                             instance=builderinformation)
        personal_form = PersonalSGAAddForm(request.POST, request.FILES,
                                           instance=sgalabel)
        ...
        if complementsga_form_ok and sgabuilderinfo_form_ok and personal_form_ok:
            return redirect(reverse("sga:sgalabel_step_two",
