...
class SGAAcademicTest(TestCase):
    ...
    def test_update_substance(self):
        ...
        response = self.client.post(
            reverse("sga:update_substance", kwargs=url), data=data
        )
    ...
    def test_delete_substance(self):
        ...
        self.assertEqual(response.status_code, 302)
        self.assertTrue(
            Substance.objects.filter(organization__pk=self.url_attr["org_pk"]).count()
            == 0
        )
        self.assertRedirects(
            response, reverse("sga:get_substance", kwargs=self.url_attr)
        )
    ...
    def test_detail_substance(self):
        ...
        self.assertTrue(response.context["object"] is not None)

    def test_add_observation(self):
        url = self.url_attr.copy()
        url["substance"] = 134
        response = self.client.post(
            reverse("sga:add_observation", kwargs=url), data={"description": "Simple"}
        )
    ...
    def test_update_observation(self):
        ...
        url = self.url_attr.copy()
        response = self.client.post(
            reverse("sga:update_observation", kwargs=url),
        ...
        self.assertTrue(self.client.session["step"] == 2)
        self.assertTrue(json.loads(response.content)["status"] is True)
    ...
    def test_delete_observation(self):
        ...
        response = self.client.post(
            reverse("sga:delete_observation", kwargs=url), data={"pk": obs.pk}
        )
    ...
    def test_getlist_substance(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertTrue(response.context["showapprove"] is False)
    ...
    def test_approve_substance(self):
        ...
        self.assertEqual(response.status_code, 302)
        self.assertRedirects(
            response, reverse("sga:approved_substance", kwargs=self.url_attr)
        )
