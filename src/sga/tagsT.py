class TextBoxTag:
    ...
    def __init__(self, props):
        self.properties = {
            ...
            "charSpacing": "letter-spaclefting",
            "minWidth": "min-width",
            "styles": {},
        }
...
class TagStyleParser(TextBoxTag, ImageTag, LineTag, ITextBoxTag):
    ...
    def set_tag(self):
        if self.type == "textbox":
            self.tag = '<p style="%s">%s</p>' % (
                self.parse_data(),
                self.convert_header(self.json_props["text"]),
            )
        if self.type == "image":
            self.tag = '<img style="%s" src="%s">' % (
                self.parse_data(),
                self.json_props["src"],
            )
        if self.type == "i-text":
            self.tag = '<p style="%s">%s</p>' % (
                self.parse_data(),
                self.json_props["text"],
            )
        if self.type == "line":
            self.tag = '<hr style="%s;%s">' % (
                self.parse_data(),
                self.get_hr_specific_styles(),
            )
        return self.tag
    ...
    def parse_data(self):
        ...
        if self.json_props["scaleX"] and self.json_props["scaleY"]:
            if "src" in self.json_props:
                ...
                if (extra_width + left) >= self.width:
                    print(self.width, extra_width + left)
                    self.styles += "transform: scale({},{});".format(
                        self.json_props["scaleX"], self.json_props["scaleY"]
                    )
                    ...
                    self.styles += "{}:{};".format(
                        "width",
                        str((self.json_props["width"] * self.json_props["scaleX"]))
                        + "px",
                    )
                    self.styles += "{}:{};".format(
                        "height",
                        str(self.json_props["height"] * self.json_props["scaleY"])
                        + "px",
                    )
                if "text" in self.json_props:
                    ...
                    self.styles += "{}:{};".format(
                        "text-align", self.json_props["textAlign"]
                    )
        ...
        if self.json_props["backgroundColor"] == "":
            ...
            self.styles += "{}:{};".format(
                "background-color", self.json_props["backgroundColor"]
            )
    ...
    def conversion_fontsize(self):
        ...
        aux = float(self.json_props["fontSize"])
    ...
    def convert_header(self, text):
        ...
        if text.find("Consejos de Prudencia") > -1:
            ...
            x, y = len("Consejos de Prudencia"), len(text)
            data += text[x:y]
        if text.find("Indicaciones de Peligro") > -1:
            data = "<strong>Indicaciones de Peligro</strong><br>"
            x, y = len("Indicaciones de Peligro"), len(text)
            data += text[x:y]
        return data

    def getY(self, height):
        result = self.json_props["top"]
        return result

    def getX(self, width):

        result = self.json_props["left"]
