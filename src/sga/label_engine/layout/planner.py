...
class LabelPlanner:
    ...
    def plan(self, bp: LabelBlueprint, ancho_px: int, alto_px: int) -> LayoutResult:
        ...
        ctx = _Ctx(bp=bp, m=m, ancho=ancho_px, alto=alto_px, dim_ref=dim_ref,
                   margen=margen, margen_int=margen_int, boxes=boxes, warnings=warnings)
    ...
    def _negotiate_scale(self, ctx: '_Ctx') -> None:
        ...
        floor_font = m.font(self._min_phrase_font(m, ctx.bp.dpi, ctx.dim_ref), bold=False)
        ...
        steps = [1.0 - 0.05 * i for i in range(int((1.0 - config.MIN_BODY_SCALE) / 0.05) + 1)]
        for s in steps:
        ...
        import math
        needed = header_end + self._top_stack_height(ctx, config.MIN_BODY_SCALE) \
            + min_phrase_h + (ctx.alto - footer_top)
    ...
    def _footer_top(self, ctx: '_Ctx') -> int:
        ...
        if bp.recipiente_nombre and bp.recipiente_color:
            rec = m.text_size(bp.recipiente_nombre,
                              m.font(int(ctx.dim_ref * 0.016), bold=True))[1] + int(ctx.alto * 0.018)
    ...
    def _top_stack_height(self, ctx: '_Ctx', scale: float) -> int:
        ...
        if bp.formula or bp.cas or bp.concentracion:
            ...
            h += hm + (int(hm * 1.3) if bp.concentracion else 0) + int(ctx.margen_int * 0.8)
    ...
    def _plan_header(self, ctx: '_Ctx') -> int:
        ...
        if logo_una:
            ...
            ctx.boxes.append(Box(ctx.margen, y_logo, w_una, alto_logo, 'logo', logo_izq_key))
        ...
        if logo_quimica:
            ...
            x_der -= w_quimica
            ctx.boxes.append(Box(x_der, y_logo, w_quimica, alto_logo, 'logo', logo_der_key))
        ...
        ctx.boxes.append(Box(
            0, y_inst, ctx.ancho, alto_texto, 'text',
            {'lines': lines, 'font_size': font_size, 'bold': True,
             'color': config.INSTITUTIONAL_COLOR, 'align': 'center',
             'line_gap': ctx.margen_int // 2},
        ))

        y = y_logo + alto_logo + ctx.margen_int
        ctx.boxes.append(Box(ctx.margen, y, ctx.ancho - 2 * ctx.margen, 1, 'rule',
                             {'color': '#CCCCCC'}))
    ...
    def _plan_name(self, ctx: '_Ctx', y: int) -> int:
        ...
        ctx.boxes.append(Box(
            ctx.margen, y, max_ancho, alto, 'name',
            {'lines': lineas, 'suffix': suffix, 'truncated': truncated,
             'font_size_main': fs_main, 'font_size_sub': fs_sub, 'line_gap': lh},
        ))
    ...
    def _plan_formula(self, ctx: '_Ctx', y: int) -> int:
        ...
        ctx.boxes.append(Box(
            ctx.margen, y, ctx.ancho - 2 * ctx.margen, int(h_main * 2.5), 'formula',
            {'formula': bp.formula, 'cas': bp.cas, 'concentracion': bp.concentracion,
             'estado_suffix': suffix if bp.formula else '',
             'font_size_main': fs_n, 'font_size_sub': fs_s, 'font_size_sup': fs_s,
    ...
    @staticmethod
    def _fit_font_width(m: Measurer, text: str, max_w: int, max_fs: int,
                        bold: bool = True, min_fs: int = 8) -> int:
    ...
    def _name_width(self, ctx: '_Ctx', scale: float) -> int:
        ...
        if suffix:
            ...
            w = max(w, m.text_width(lineas[-1], font_main) + m.text_width(suffix, font_sub) + 4)
        return w
    ...
    def _plan_signal_topright(self, ctx: '_Ctx', y0: int, y1: int) -> None:
        ...
        color = (config.BORDER_COLORS['PELIGRO'] if palabra.upper() == 'PELIGRO'
                 else config.BORDER_COLORS.get('ATENCION', '#FFA500'))
        ctx.boxes.append(Box(
            x, yy, fw, fh, 'text',
            {'lines': [palabra], 'font_size': fs, 'bold': True,
             'color': color, 'align': 'left', 'line_gap': 0},
        ))
    ...
    def _hazard_metrics(self, ctx: '_Ctx', scale: float) -> dict:
        ...
        """
        m, bp = ctx.m, ctx.bp
        osha = [s for s in bp.simbolos if s in config.PICTOGRAMAS_OSHA][:config.MAX_OSHA_PICTOGRAMS]
        n = len(osha)
        ...
        d = {
            'osha': osha, 'tam_picto': tam_picto, 'row_h': row_h,
            'x_pictos_end': x_pictos_end, 'x_qr': x_qr,
            'palabra': bp.palabra_advertencia, 'sig_below': False,
            'sig_fs': 0, 'sig_sh': 0, 'sig_x': ctx.margen, 'sig_w': 0,
        }
        palabra = bp.palabra_advertencia
        if palabra and not ctx.sig_topright:
            ...
            if row_h and fs_beside >= min_prominente:
                d.update(sig_below=False, sig_fs=fs_beside,
                         sig_sh=m.text_size(palabra, m.font(fs_beside, bold=True))[1],
                         sig_x=x_beside, sig_w=avail_beside)
            else:
                # Propia línea debajo, a ancho completo (también crece).
                avail_below = max(int(ctx.dim_ref * 0.1), x_qr - ctx.margen_int - ctx.margen)
                ...
                d.update(sig_below=True, sig_fs=fs_below,
                         sig_sh=m.text_size(palabra, m.font(fs_below, bold=True))[1],
                         sig_x=ctx.margen, sig_w=avail_below)
    ...
    def _plan_hazard(self, ctx: '_Ctx', y: int) -> int:
        ...
        for simb in d['osha']:
            ctx.boxes.append(Box(x_picto, picto_y, tam_picto, tam_picto, 'pictogram',
                                 {'symbol': simb}))
        ...
        if palabra and not ctx.sig_topright:
            ...
            ctx.boxes.append(Box(
                d['sig_x'], sig_y, d['sig_w'], d['sig_sh'], 'text',
                {'lines': [palabra], 'font_size': d['sig_fs'], 'bold': True,
                 'color': color, 'align': 'left', 'line_gap': 0},
            ))
        ...
        if epp:
            ...
            y_epp = epp_top
            for simb in epp:
                ctx.boxes.append(Box(x_center, y_epp, tam_epp, tam_epp, 'pictogram',
                                     {'symbol': simb}))
    ...
    def _plan_phrases_and_footer(self, ctx: '_Ctx', y: int) -> None:
        ...
        if recipiente_nombre and recipiente_color:
            alto_texto_rec = m.text_size(recipiente_nombre, font_recipiente)[1] + int(ctx.alto * 0.018)
        ...
        if y_frases_end - y_frases_start > int(ctx.alto * 0.1):
            ...
            if h_items or p_items:
                plan = self._fit_phrase_block(m, h_items, p_items, max_width, avail_h, ctx.dim_ref, bp.dpi)
                ctx.boxes.append(Box(
                    x_min, y_frases_start, max_width, avail_h, 'phrases', plan,
                ))
            ...
            if bp.info_adicional and y_actual < y_frases_end:
                ...
                if fit_lines:
                    ctx.boxes.append(Box(
                        x_min, y_actual, max_width, yy - y_actual, 'text',
                        {'lines': fit_lines, 'font_size': fs_info, 'bold': False,
                         'color': config.INFO_COLOR, 'align': 'center',
        ...
        if textos_pie:
            ...
            if cur:
                pie_lines.append("  •  ".join(cur))
            ctx.boxes.append(Box(
                ctx.margen, y_pie1, ctx.ancho - 2 * ctx.margen, altura_linea * len(pie_lines), 'text',
                {'lines': pie_lines, 'font_size': fs_small, 'bold': True,
                 'color': config.MISC_COLOR, 'align': 'left',
        ...
        ctx.boxes.append(Box(
            ctx.margen, y_prep, ctx.ancho - 2 * ctx.margen, altura_prep, 'text',
            {'lines': textos_prep, 'font_size': fs_small, 'bold': True,
             'color': config.MISC_COLOR, 'align': 'left',
        ...
        if recipiente_nombre and recipiente_color:
            ...
            ctx.boxes.append(Box(
                0, y_rec, ctx.ancho, th, 'text',
                {'lines': [recipiente_nombre], 'font_size': fs_recipiente, 'bold': True,
                 'color': recipiente_color, 'align': 'center', 'line_gap': 0},
            ))
    ...
    def _plan_border(self, ctx: '_Ctx') -> None:
        ...
        ctx.boxes.append(Box(
            0, 0, ctx.ancho, ctx.alto, 'border',
            {'palabra_advertencia': ctx.bp.palabra_advertencia, 'thickness': thickness},
            z_index=10,
        ))
        ...
        if ctx.bp.recipiente_color:
            ...
            ctx.boxes.append(Box(
                0, 0, ctx.ancho, ctx.alto, 'accent',
                {'color': ctx.bp.recipiente_color, 'thickness': acc_thickness,
                 'inset': thickness},
                z_index=9,
            ))
    ...
    def _fit_phrase_block(self, m: Measurer, h_items, p_items, max_width, avail_h,
                          dim_ref, dpi) -> dict:
        ...
        if p_codes:
            ...
            if stacked(h_full_lines, p_lines, fds_lines, spacing=lh, gap=gap) <= avail_h:
                return self._make_plan(min_font, lh, gap, h_full_lines, p_lines, fds_lines)

        # Fase 3: H completo + FDS (sin P; la FDS cubre la prudencia).
        if stacked(h_full_lines, fds_lines, spacing=lh, gap=gap) <= avail_h:
            return self._make_plan(min_font, lh, gap, h_full_lines, [],
                                   fds_lines if p_items else None)
        ...
        if p_codes:
            p_lines = m.wrap_items([p_codes], max_width, font)
            if stacked(h_code_lines, p_lines, fds_lines, spacing=lh, gap=gap) <= avail_h:
                return self._make_plan(min_font, lh, gap, h_code_lines, p_lines, fds_lines)
        ...
        return self._make_plan(min_font, lh, gap, h_code_lines[:max_lines], [], fds_lines)

    @staticmethod
    def _make_plan(font_size, lh, gap, h_lines, p_lines, fds_lines) -> dict:
        return {
            'font_size': font_size, 'line_height': lh, 'gap': gap,
            'inner': int(lh * 0.25),
            'h_lines': h_lines, 'p_lines': p_lines,
...
class _Ctx:
    ...
    __slots__ = ('bp', 'm', 'ancho', 'alto', 'dim_ref', 'margen', 'margen_int',
                 'boxes', 'warnings', 'scale', 'sig_topright')

    def __init__(self, bp, m, ancho, alto, dim_ref, margen, margen_int, boxes, warnings):
        self.bp = bp
        self.m = m
        self.ancho = ancho
        self.alto = alto
