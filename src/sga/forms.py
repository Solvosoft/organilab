...
class PersonalTemplateForm(GTForm):
    name = forms.CharField(
        max_length=100, required=True, widget=genwidgets.TextInput, label=_("Name")
    )
    template = forms.ModelChoiceField(
        queryset=TemplateSGA.objects.none(),
        required=True,
...
class PersonalEditorForm(forms.ModelForm, GTForm):
    recipient_size = forms.ModelChoiceField(
...
class SearchDangerIndicationForm(GTForm):
    codes = forms.ModelMultipleChoiceField(
        queryset=DangerIndication.objects.all().exclude(code="Ninguno"),
...
class PersonalForm(GTForm):
    ...
    company_name = forms.CharField(
        max_length=150, required=True, widget=genwidgets.TextInput
    )
    ...
    substance = forms.ModelChoiceField(
        queryset=Substance.objects.all(), widget=genwidgets.Select
    )
    commercial_information = forms.CharField(widget=genwidgets.Textarea)
    barcode = forms.CharField(
        widget=genwidgets.TextInput, max_length=150, required=False
    )
...
class PersonalSGAForm(forms.ModelForm, GTForm):
    recipient_size = forms.ModelChoiceField(
        widget=genwidgets.Select, queryset=RecipientSize.objects.all(), required=True
    )
...
class LabelForm(forms.ModelForm, GTForm):
    class Meta:
        ...
        widgets = {"substance": genwidgets.Select}
    ...
    class Meta:
        ...
        widgets = {
            ...
            "provider": genwidgets.Select(
                attrs={
                    "data-dropdownparent": "#provider_modal",
...
class SGALabelBuilderInformationForm(forms.ModelForm, GTForm):
    company = forms.ModelChoiceField(
        widget=genwidgets.Select(),
        queryset=BuilderInformation.objects.none(),
        required=False,
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        if user:
            self.fields["company"].queryset = BuilderInformation.objects.filter(
                user=user
            )
