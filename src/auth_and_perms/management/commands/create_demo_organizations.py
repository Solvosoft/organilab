...
from auth_and_perms.views.user_org_creation import set_rol_administrator_on_org, create_user_organization
from laboratory.models import OrganizationStructure, Laboratory, UserOrganization, OrganizationStructureRelations
...
class Command(BaseCommand):
    ...
    org_contenttype = ContentType.objects.filter(app_label='laboratory', model='organizationstructure').first()
    lab_contenttype = ContentType.objects.filter(app_label='laboratory', model='laboratory').first()
    ...
    def create_user_organization(self, organization, num_org=1, appenduser=[], usertype=UserOrganization.ADMINISTRATOR):
        users=[]
        for num in range(num_org):
            user = User.objects.create_user('user_%d_%d'%(num, organization.pk), fake.free_email(), 'admin12345')
    ...
    def create_user_lab_organization(self, organization, laboratory, num_org=1):
        for num in range(num_org):
            user = User.objects.create_user('user_%d_%d_%d'%(num, organization.pk, laboratory.pk),
                                            fake.free_email(), 'admin12345')
            user.first_name=fake.first_name_nonbinary()
            user.last_name=fake.last_name()
            user.save()
            ...
            UserOrganization.objects.create(organization=organization, user=user,
    ...
    def create_organizationstructurerelations_laboratory(self, organization, lab):
        ...
        print( "OrganizationStructureRelations ", organization.pk, lab.pk, self.lab_contenttype)
    def create_organizations(self):
        for org_num in range(self.num_of_orgs):
            org = OrganizationStructure.objects.create( name="Mi org %d"%org_num, level=0, position=0)
            ...
            self.create_laboratory(self.lab_per_org, org, post_name="%d"%org_num, user_per_lab=self.user_per_lab_org)
            for children_num in range(self.num_of_children):
                ...
                self.create_user_organization(org_chi, num_org=self.user_per_children, appenduser=users,
                                              usertype=UserOrganization.LABORATORY_MANAGER)
                self.create_organizationstructurerelations(org_chi)
                self.create_laboratory(self.lab_per_children, org_chi, post_name="%d - %d" % (org_num, children_num),
                ...
                for cousin_num in range(self.num_of_cousin):
                    org_cousin = OrganizationStructure.objects.create(
                                    name="Mi org %d %d %d - %d -- %d" % (org.pk, org_chi.pk, org_num, children_num,
                                                                         cousin_num),
                                    level=2,
                                    parent=org_chi,
                                    position=2
                    ...
                    self.create_user_organization(org_cousin, num_org=self.user_per_cousing, appenduser=users,
                                              usertype=UserOrganization.LABORATORY_MANAGER)
                    self.create_laboratory(self.lab_per_cousin, org_cousin,
