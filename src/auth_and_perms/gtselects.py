    ...
    def filter_queryset(self, queryset):
        ...
        if as_role:
            ...
            if profilepermission:
                self.selected = [
                    str(idpp)
                    for idpp in profilepermission.rol.all().values_list("id", flat=True)
                ]
        return queryset
            ...
            if form.is_valid():
                ...
                if form.cleaned_data["typeofcontenttype"] == "laboratory":
                    if form.cleaned_data["laboratory"]:
                        self.contenttypeobj = get_object_or_404(
                            Laboratory.objects.using(settings.READONLY_DATABASE),
                            pk=form.cleaned_data["laboratory"],
                        )
                        organization_can_change_laboratory(
                    ...
                    self.contenttypeobj = self.organization
                    user_is_allowed_on_organization(
                        self.request.user, self.contenttypeobj
                    )
            else:
                return Response(
                    {
...
class UserS2OrgManagement(generics.RetrieveAPIView, BaseSelect2View):
    ...
    def filter_queryset(self, queryset):
        ...
        if profiles:
            self.selected = [
                str(idpp) for idpp in profiles.values_list("user__id", flat=True)
            ]
        return queryset
...
class LabOrgBaseS2(generics.RetrieveAPIView, BaseSelect2View):
    ...
    def get_queryset(self):
        ...
        labs += list(
            self.organization.laboratory_set.using(settings.READONLY_DATABASE)
            .all()
...
class OrgbyUserOrgS2(BaseSelect2View):
    ...
    def get_queryset(self):
        ...
        orgByuser = OrganizationStructure.os_manager.filter_user(
            self.request.user, ancestors=True
        )
        if org:
        ...
        return self.model.objects.filter(
            pk__in=tuple(orgByuser.values_list("pk", flat=True))
        )
