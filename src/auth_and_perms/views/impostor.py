...
def add_user_impostor(request, org_pk, pk):
    ...
    user_in_org = UserOrganization.objects.filter(
        organization=org_pk, user=imposted_as
    ).exists()
    if user_in_org:
        if request.user.pk != imposted_as.pk:
            ...
            if not exist_instance:
                ...
                messages.error(
                    ...
                    mark_safe(
                        _(
                            'You have active impostor session <a class="text-white fw-bold text-decoration-underline" href="%(reverseurl)s?all=1">Finish it</a>'
                        )
    ...
    if delall:
        if hasattr(request, "impostor_info"):
            ImpostorLog.objects.filter(
                impostor=request.impostor_info.impostor, active=True
            ).update(active=False, logged_out=now())
        else:
            ImpostorLog.objects.filter(impostor=request.user, active=True).update(
                active=False, logged_out=now()
            )
    else:
        ipaddress = get_ip_address(request)
        ImpostorLog.objects.filter(
            imposted_as=request.user, impostor_ip=ipaddress
        ).update(active=False, logged_out=now())
