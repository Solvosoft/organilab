...
class AddRolForm(GTForm, forms.Form):
    rols = forms.ModelMultipleChoiceField(
        queryset=Rol.objects.all(),
        label=_("Rols"),
        widget=AutocompleteSelectMultiple("roluserorgbase"),
        required=True,
    )
...
class CreationUserOrganization(UserCreationForm, GTForm):
    organization_name = forms.CharField(
        max_length=255, widget=genwidgets.TextInput, label=_("Organization name")
    )
    ...
    class Meta(UserCreationForm.Meta):
        ...
        widgets = {"username": genwidgets.EmailInput}


# from django_otp.views import LoginView
class AddProfileForm(OTPTokenForm, GTForm):
    first_name = forms.CharField(
        label=_("First name"), max_length=150, widget=genwidgets.TextInput
    )
    last_name = forms.CharField(
        label=_("Last name"), max_length=150, widget=genwidgets.TextInput
    )
    email = forms.EmailField(label=_("Email address"), widget=genwidgets.EmailInput)
    phone_number = forms.CharField(
        label=_("Phone"), max_length=25, widget=genwidgets.TextInput
    )
    id_card = forms.CharField(
        label=_("Identification"), max_length=100, widget=genwidgets.TextInput
    )
    job_position = forms.CharField(
        label=_("Job Position"), max_length=100, widget=genwidgets.TextInput
    )
...
class OrganizationActions(GTForm):
    ...
    def clean(self):
        ...
        if organization and not organization.active:
            ...
            if actions in [1, 3]:
                self.add_error(
                    "action_organization", _("Organization cannot be inactive")
                )
...
class SearchObjByOrgForm(GTForm):
    object = forms.ModelChoiceField(
        ...
        widget=AutocompleteSelect(
            "objbyorg", attrs={"data-s2filter-organization": "#id_organization"}
        ),
