...
class OrganizationTest(TestCase):
    ...
    def test_add_user_organization(self):
        ...
        pre_user_org = UserOrganization.objects.get(
            user__pk=1, organization__pk=1
        ).status

        response = self.client.post(
            reverse("auth_and_perms:addusersorganization", kwargs={"pk": 1}), data=data
        )

        self.assertEqual(response.status_code, 302)
        pos_user_org = UserOrganization.objects.get(
            user__pk=1, organization__pk=1
        ).status
    ...
    def test_remove_user_organization(self):
        ...
        pre_user_org = UserOrganization.objects.get(
            user__pk=1, organization__pk=1, type_in_organization=1
        ).status
        ...
        self.assertEqual(response.status_code, 302)
        pos_user_org = UserOrganization.objects.filter(
            user__pk=1, organization__pk=1
        ).count()
    ...
    def test_user_organization_error_pk(self):
        ...
        response = self.client.get(
            reverse("auth_and_perms:addusersorganization", kwargs={"pk": 1})
        )

        self.assertEqual(response.status_code, 302)
        self.assertTrue(
            _("Organization doesn't exists")
    ...
    def test_adduser_organization(self):
        ...
        response = self.client.post(
            reverse("auth_and_perms:add_user", kwargs={"pk": 1}), data=data
        )
        self.assertEqual(response.status_code, 302)
        user = User.objects.last()
        user_org = UserOrganization.objects.filter(
            user=user, organization__pk=1
        ).first()
    ...
    def test_list_rol_organization(self):
        ...
        response = self.client.get(
            reverse("auth_and_perms:list_rol_by_org", kwargs={"org_pk": 1})
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_delete_rol_organization(self):
        get_response = self.client.get(
            reverse("auth_and_perms:del_rol_by_org", kwargs={"org_pk": 1, "pk": 1})
        )
    ...
    def test_copy_rols(self):
        ...
        data = {"org_pk": 1, "rols": [3]}
        pre_rol = OrganizationStructure.objects.get(pk=1).rol.count()
        post_response = self.client.post(
            reverse("auth_and_perms:copy_rols", kwargs={"pk": 1}), data=data
        )
        self.assertEqual(post_response.status_code, 302)
        pos_rol = OrganizationStructure.objects.get(pk=1).rol.count()
        success_url = reverse("auth_and_perms:organizationManager")
        self.assertTrue(
            _("Element saved successfully")
            == str(list(get_messages(post_response.wsgi_request))[0])
        )
    ...
    def test_add_contenttype_to_org(self):
        data = {"organization": 1, "contentyperelobj": [1, 2]}
        org_relacion = (
            OrganizationStructureRelations.objects.all().count()
        )  # No rel on auth.json
        response = self.client.post(
            reverse("auth_and_perms:add_contenttype_to_org"), data=data
        )
    ...
    def test_add_contenttype_to_org_not_lab(self):
        data = {"organization": 1, "contentyperelobj": []}
    ...
    def test_add_contenttype_to_org_error(self):
        data = {"organization": 999, "contentyperelobj": [37]}
        response = self.client.post(
            reverse("auth_and_perms:add_contenttype_to_org"), data=data
        )
        org_relations = OrganizationStructureRelations.objects.filter(
