...
from auth_and_perms.api.serializers import (
    ...
    ValidateLabOrgObjectSerializer,
...
class ObjectGModelLookup(BaseSelect2View):
    ...
    def list(self, request, *args, **kwargs):
        ...
        self.serializer = ValidateUserAccessShelfTypeSerializer(
            data=request.GET, context={"user": request.user}
        )
        if self.serializer.is_valid():
...
class CatalogUnitLookup(BaseSelect2View):
    ...
    def list(self, request, *args, **kwargs):
        ...
        self.serializer = ValidateUserAccessShelfSerializer(
            data=request.GET, context={"user": request.user}
        )
        if self.serializer.is_valid():
            self.shelf = self.serializer.validated_data["shelf"]
...
@register_lookups(
    prefix="available-container-search", basename="available-container-search"
)
class AvailableContainerLookup(BaseSelect2View):
    ...
    def get_text_display(self, obj):
        text_display = (
            f"["
            + _("Shelf")
            + f" {obj.shelf.name}] - {obj.object.code} {obj.object.name}"
        )
    ...
    def get_queryset(self):
        if self.laboratory and self.shelf:
            queryset = get_available_containers_for_selection(
                self.laboratory.pk, self.shelf.pk
            )
        else:
...
@register_lookups(
    prefix="container-for-cloning-search", basename="container-for-cloning-search"
)
...
@register_lookups(
    prefix="shelfobject_status_search", basename="shelfobject_status_search"
)
...
class ObjectProvidersLookup(BaseSelect2View):
    ...
    def get_queryset(self):
        ...
        if self.org_pk and self.obj and hasattr(self.obj, "equipmentcharacteristics"):
            queryset = queryset.filter(
                pk__in=self.obj.equipmentcharacteristics.providers.values_list(
                    "pk", flat=True
                )
            )
        else:
    ...
    def list(self, request, *args, **kwargs):
        self.serializer = ValidateLabOrgObjectSerializer(
            data=request.GET, context={"user": request.user}
        )
...
class InstrumentalFamilyLookup(BaseSelect2View):
    ...
    def list(self, request, *args, **kwargs):
        self.serializer = UserAccessOrgLabValidateSerializer(
            data=request.GET, context={"user": request.user}
        )
        if self.serializer.is_valid():
            return super().list(request, *args, **kwargs)
...
class EquipmentTypeLookup(BaseSelect2View):
    ...
    def list(self, request, *args, **kwargs):
        serializer = UserAccessOrgLabValidateSerializer(
            data=request.GET, context={"user": request.user}
        )
