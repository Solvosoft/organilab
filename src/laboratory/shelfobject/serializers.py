...
class ContainerSerializer(serializers.Serializer):
    ...
    def get_fields(self, *args, **kwargs):
        ...
        if container_select_option == 'clone':
            ...
            fields[
                'available_container'].queryset = get_available_containers_for_selection(
                self.context['laboratory_id'])
...
class IncreaseShelfObjectSerializer(serializers.Serializer):
    ...
    def validate(self, data):
        ...
        if check_measurement_unit:
            logger.debug(
                f'IncreaseShelfObjectSerializer --> shelf_object.measurement_unit != '
                f'shelf.measurement_unit and shelf.measurement_unit ({check_measurement_unit})')
            raise serializers.ValidationError({'shelf_object': _(
        ...
        if total > shelf_quantity and not shelf.infinity_quantity:
            logger.debug(
                f'IncreaseShelfObjectSerializer --> total ({total}) > shelf_quantity ({shelf_quantity}) '
...
class MoveShelfObjectSerializer(serializers.Serializer):
    ...
    def validate(self, data):
        ...
        if shelf.measurement_unit != shelf_object.measurement_unit:
            logger.debug(
                f'MoveShelfObjectSerializer --> shelf ({shelf.pk}) == shelf_object.shelf.pk ({shelf_object.shelf.pk})')
            raise serializers.ValidationError({'shelf': _(
                "Object cannot be moved to a shelf with different measurement unit.")})
...
class TransferInShelfObjectApproveWithContainerSerializer(
    ...
    def validate(self, data):
        ...
        if container_select_option == "use_source" and transfer_object.quantity < transfer_object.object.quantity:
            print(transfer_object.quantity)
            print(transfer_object.object.quantity)
            logger.debug(
