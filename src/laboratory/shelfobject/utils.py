...
def build_shelfobject_qr(request, shelfobject, organization, laboratory):
    qr, url = get_or_create_qr_shelf_object(
        request, shelfobject, organization, laboratory
    )
...
def create_shelfobject_observation(
    shelfobject, description, action_taken, user, laboratory
):
    observation = ShelfObjectObservation.objects.create(
        description=description,
        action_taken=action_taken,
        shelf_object=shelfobject,
        created_by=user,
...
def get_available_containers_for_selection(laboratory_id, shelf_id):
    ...
    shelf = get_object_or_404(
        Shelf.objects.using(settings.READONLY_DATABASE), pk=shelf_id
    )
...
def get_or_create_container_based_on_selected_option(
    container_selected_option,
    ...
    if container_selected_option == "clone":
        container = create_new_shelfobject_from_object_in(
        ...
        container = move_shelfobject_to(
            source_shelfobject.container,
            destination_organization_id,
            destination_laboratory_id,
            destination_shelf,
            request,
            observation_text="Moved Container",
        )
    elif container_selected_option == "new_based_source":
        container = clone_shelfobject_to(
            source_shelfobject.container,
            destination_organization_id,
            destination_laboratory_id,
            destination_shelf,
...
def move_shelfobject_partial_quantity_to(
    shelfobject,
    destination_organization_id,
    destination_laboratory_id,
    destination_shelf,
    ...
    update_shelfobject_quantity(
        ...
        original_shelfobject.quantity - previous_quantity,
        request.user,
        organization=destination_organization_id,
    )
...
def create_new_shelfobject_from_object_in(
    object,
    destination_organization_id,
    destination_laboratory_id,
    destination_shelf,
    request,
    measurement_unit_description="Unidades",
):
    measurement_unit = get_object_or_404(
        Catalog, key="units", description=measurement_unit_description
    )
...
def get_lab_room_queryset_by_filters(queryset, shelfobject, key, filters):
    available_labrooms = get_available_objs_by_shelfobject(
        queryset, shelfobject, key, filters
    )
    filters_lab_room = (
        Q(
            furniture__shelf__measurement_unit__isnull=True,
            furniture__shelf__infinity_quantity=True,
        )
        | Q(
            furniture__shelf__measurement_unit=shelfobject.measurement_unit,
            furniture__shelf__infinity_quantity=True,
        )
...
def get_furniture_queryset_by_filters(queryset, shelfobject, key, filters):
    available_furnitures = get_available_objs_by_shelfobject(
        queryset, shelfobject, key, filters
    )
    filters_furniture = (
        Q(shelf__measurement_unit__isnull=True, shelf__infinity_quantity=True)
        | Q(
            shelf__measurement_unit=shelfobject.measurement_unit,
...
def get_shelf_queryset_by_filters(queryset, shelfobject, key, filters):
    available_shelves = get_available_objs_by_shelfobject(
        queryset, shelfobject, key, filters
    )

    filters_shelves = (
        Q(measurement_unit__isnull=True, infinity_quantity=True)
        | Q(measurement_unit=shelfobject.measurement_unit, infinity_quantity=True)
...
def validate_measurement_unit_and_quantity(
    ...
    total = (
        shelf.get_total_refuse(
            include_containers=False, measurement_unit=shelf.measurement_unit
        )
        + quantity
    )

    shelfbaseunit = BaseUnitValues.objects.filter(
    ...
    if (
        measurement_unit
        and shelf.measurement_unit
        and measurement_unit != shelf.measurement_unit
    ):

        if (
            baseunit.measurement_unit_base
            and shelfbaseunit.measurement_unit_base
            and baseunit.measurement_unit_base != shelfbaseunit.measurement_unit_base
        ):
...
def group_object_errors_for_serializer(
    errors,
    save_to_key="shelf_object",
    keys_to_group=("quantity", "object", "measurement_unit"),
):
    object_errors = []
    updated_errors = {}
