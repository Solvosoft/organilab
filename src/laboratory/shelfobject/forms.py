...
class TransferOutShelfObjectForm(GTForm):
    ...
    mark_as_discard = forms.BooleanField(
        widget=genwidgets.YesNoInput, required=False, label=_("Mark as discard")
    )
...
class MoveShelfObjectForm(GTForm):
    ...
    shelf = forms.ModelChoiceField(
        ...
        help_text=_(
            "This select only shows shelves with same measurement unit than current object"
        ),
    )
    shelf_object = forms.IntegerField(widget=forms.HiddenInput)
...
class ShelfObjectExtraFields(GTForm, forms.Form):
    ...
    without_limit = forms.BooleanField(
        widget=genwidgets.CheckboxInput(attrs={"class": "check_limit"}),
        label=_("Unlimited"),
        initial=True,
    )
    minimum_limit = forms.FloatField(
        widget=genwidgets.TextInput,
        required=True,
        initial=0.0,
        label=_("Minimum Limit"),
    )
    maximum_limit = forms.FloatField(
        widget=genwidgets.TextInput,
        required=True,
        initial=0.0,
...
class ShelfObjectMaterialForm(ShelfObjectExtraFields, forms.ModelForm, GTForm):
    ...
    class Meta:
        ...
        widgets = {
            "shelfobject_code": genwidgets.TextInput,
...
class EquipmentCharacteristicForm(forms.Form):
    provider = forms.ModelChoiceField(
        queryset=Provider.objects.all(), widget=genwidgets.Select, label=_("Provider")
    )
    authorized_roles_to_use_equipment = forms.ModelMultipleChoiceField(
        queryset=Rol.objects.none(),
        widget=genwidgets.SelectMultiple,
        label=_("Authorized roles to use equipment"),
    )
    equipment_price = forms.FloatField(
        initial=0.0,
        label=_("Price"),
        widget=genwidgets.TextInput,
        help_text=_("Use dot like 0.344 on decimal"),
    )
    purchase_equipment_date = forms.DateField(
        widget=genwidgets.DateInput, label=_("Purchase Date")
    )
    delivery_equipment_date = forms.DateField(
        widget=genwidgets.DateInput, label=_("Delivery Date")
    )
    have_guarantee = forms.BooleanField(
        initial=False, widget=genwidgets.YesNoInput, label=_("Has guarantee?")
    )
    contract_of_maintenance = forms.FileField(
        widget=FileChunkedUpload, label=_("Contract of maintenance")
    )
    available_to_use = forms.BooleanField(
        initial=False, widget=genwidgets.YesNoInput, label=_("Is available to use?")
    )
    first_date_use = forms.DateField(
        widget=genwidgets.DateInput, label=_("First date use")
    )
...
class ShelfObjectRefuseEquipmentForm(
    EquipmentCharacteristicForm, GTForm, forms.ModelForm
):
    objecttype = forms.IntegerField(
        widget=genwidgets.HiddenInput, min_value=0, max_value=3, required=True
    )
...
class ContainerForm(GTForm):
    container_select_option = forms.ChoiceField(
        widget=genwidgets.RadioVerticalSelect,
        choices=ContainerSerializer.CONTAINER_SELECT_CHOICES,
        label=_("Container Options"),
    )
    container_for_cloning = forms.ModelChoiceField(
        widget=genwidgets.Select, queryset=Object.objects.none(), label=_("Container")
    )
    available_container = forms.ModelChoiceField(
        widget=genwidgets.Select,
    ...
    def __init__(self, *args, **kwargs):
        ...
        set_container_advanced_options = kwargs.pop(
            "set_container_advanced_options", False
        )
...
class ShelfObjectReactiveForm(
    ShelfObjectExtraFields, ContainerForm, forms.ModelForm, GTForm
):
...
class ShelfObjectRefuseReactiveForm(
    ShelfObjectExtraFields, ContainerForm, GTForm, forms.ModelForm
):
...
class ShelfobjectMaintenanceForm(GTForm, forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["provider_of_maintenance"].queryset = Provider.objects.filter(
            laboratory__organization__pk=org
        )
...
class EditEquimentShelfobjectForm(forms.ModelForm, GTForm):
    shelfobject_code = forms.CharField(
        widget=genwidgets.TextInput, label=_("Unit code"), required=False
    )
    description = forms.CharField(widget=genwidgets.Textarea, label=_("Description"))
    ...
    was_donated = forms.BooleanField(
        widget=genwidgets.YesNoInput, required=False, label=_("Was donated?")
    )
