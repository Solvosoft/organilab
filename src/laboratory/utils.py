...
def check_lab_group_has_perm(
    user, lab, perm, callback_filter=filter_laboratorist_profile
):
...
def get_rols_from_organization(rootpk, rolfilters={}, org=None):
    ...
    query = OrganizationStructure.objects.filter(pk=rootpk, **rolfilters).descendants(
        of=org, include_self=True
    )
...
def find_rel_object(object):
    natural_name = "%s.%s" % (object._meta.app_label, object._meta.model_name)
    instance = None
...
def organilab_logentry(
    ...
    if not change_message:
        if action_flag != 3:
            change_message = str(
                [{action: {"fields": changed_data if changed_data else []}}]
            )
        else:
...
def get_organizations_register_user_base(organization, lab_id):
    ...
    org_list_by_lab = OrganizationStructureRelations.objects.filter(
        organization__in=org_base_list, content_type=content_type, object_id=lab_id
    )
...
def get_exclude_org_register_user(lab_pk, query_org, org_register_pk=None):
    ...
    exclude_org = RegisterUserQR.objects.filter(
        organization_register__in=query_org, content_type=content_type, object_id=lab_pk
    )
...
def get_organizations_register_user(organization, lab_id, org_register_pk=None):
    ...
    org_exclude = get_exclude_org_register_user(
        lab_id, org_base, org_register_pk=org_register_pk
    )
    return (
        OrganizationStructure.objects.filter(pk__in=org_base)
...
def save_object_by_action(user, obj, relobj, changed_data, action_flag, object_repr):
    obj.save()
    organilab_logentry(
        user, obj, action_flag, object_repr, changed_data=changed_data, relobj=relobj
    )
...
def delete_profile_roles_related_to_laboratory(
    profilepermissions_list, organization, laboratory
):
...
def get_change_message(relation_list, laboratory, relation_obj):
    change_objects = (
        str(laboratory),
        (
            str(relation_obj.organization)
            if hasattr(relation_obj, "organization")
...
def delete_relation_between_laboratory_with_other_models(
    general_relation_list, user, laboratory
):
        ...
        for relation_obj in relation_list["list"]:
            organilab_logentry(
                ...
                change_message=get_change_message(
                    relation_list, laboratory, relation_obj
                ),
                relobj=(
                    relation_obj.organization
                    if hasattr(relation_obj, "organization")
                    else laboratory
                ),
            )
...
class PermissionByLaboratoryInOrganization(BasePermission):
    ...
    def has_permission(self, request, view):
        ...
        try:
            ...
            organization_can_change_laboratory(
                view.laboratory, view.organization, raise_exec=True
            )
