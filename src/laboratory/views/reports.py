...
from report.forms import (
    ReportForm,
    ReportObjectsForm,
    ObjectLogChangeReportForm,
    OrganizationReactiveForm,
    ValidateObjectTypeForm,
    DiscardShelfForm,
    ReactiveReportForm,
    RiskZoneReportForm, ReactiveStockReportForm, ChemicalInventoryReportForm,
)
        ...
        context.update(
            {
                ...
                "form": ObjectLogChangeReportForm(
                    initial={
                        "name": slugify(
                            title + " " + now().strftime("%x").replace("/", "-")
                        ),
...
class PrecursorsView(ReportListView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context["datalist"] = PrecursorReport.objects.filter(
            laboratory__pk=int(self.lab)
        ).order_by("-pk")
        return context
        ...
        context.update(
            {
                ...
                "form": ChemicalInventoryReportForm(
                    initial={
                        "name": slugify(
                            title + " " + now().strftime("%x").replace("/", "-")
                        ),
