...
from ..shelfobject.forms import (
    ShelfobjectMaintenanceForm,
    UpdateShelfobjectMaintenanceForm,
    ShelfobjectLogForm,
    ShelfobjectCalibrateForm,
    UpdateShelfobjectCalibrateForm,
...
def list_shelfobject(request, *args, **kwargs):
    return {
        "inner-fragments": {
            "#shelfobject": list_shelfobject_render(
                request, org_pk=kwargs["org_pk"], lab_pk=kwargs["lab_pk"]
            ),
...
class ShelfObjectForm(forms.ModelForm, GTForm):
    ...
    def clean_measurement_unit(self):
        ...
        amount = (
            ...
            and (quantity + shelf.get_total_refuse()) <= shelf.quantity
        )
        if shelf.measurement_unit is None:
            return unit
        if shelf.measurement_unit == unit:
            ...
            self.add_error(
                "measurement_unit",
                _(
                    "Need add the same measurement unit that the shelf has  %(measurement_unit)s"
                )
    ...
    class Meta:
        ...
        widgets = {
            ...
            "limits": core.SelectWithAdd(
                attrs={"add_url": reverse_lazy("laboratory:add_shelfobjectlimit")}
            ),
        }
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["object"] = forms.ModelChoiceField(
            ...
            widget=AutocompleteSelect(
                ...
                url_suffix="-detail",
                url_kwargs={"pk": org_pk},
                attrs={"data-dropdownparent": "#createshelfobjectform"},
            ),
            label=objecttype,
            help_text=_("Search by name, code or CAS number"),
        )
    ...
    def clean(self):
        ...
        if shelf.measurement_unit == measurement_unit or not shelf.measurement_unit:
            ...
            if shelf.quantity >= new_total or not shelf.quantity:
                ...
                self.add_error(
                    "quantity",
                    _("The quantity is much larger than the shelf limit %(limit)s")
            ...
            self.add_error(
                "measurement_unit",
                _(
                    "The measurent unit is different of there shelf has %(measurement_unit)s"
                )
                % {"measurement_unit": shelf.measurement_unit},
            )
        return cleaned_data
...
class ShelfObjectCreate(AJAXMixin, CreateView):
    ...
    def get_success_url(self):
        return reverse_lazy(
            "laboratory:list_shelf", args=(self.org, self.lab, self.shelf.furniture.pk)
        )
    ...
    def form_valid(self, form):
        ...
        url = url + "#labroom=%d&furniture=%d&shelf=%d&shelfobject=%d" % (
            self.object.shelf.furniture.labroom.pk,
            self.object.shelf.furniture.pk,
            self.object.shelf.pk,
            self.object.pk,
        )
        self.object.shelf_object_url = url
        img, file = utils.generate_QR_img_file(
            url, self.request.user, extension_file=".svg", file_name="qrcode"
        )
...
class ShelfObjectEdit(AJAXMixin, UpdateView):
    ...
    def get_success_url(self):
        return reverse_lazy(
            "laboratory:list_shelf",
            args=(self.org, self.lab, self.object.shelf.furniture.pk),
        )

    def form_valid(self, form):
        old = self.model.objects.filter(pk=self.object.id).values("quantity")[0][
            "quantity"
        ]
        ...
        return {
            "inner-fragments": {
                "#row_%d_col_%d_shelf_%d"
                % (row, col, self.object.shelf.pk): list_shelfobject_render(
                    self.request,
                    self.object.shelf.pk,
                    row,
                    col,
...
class ShelfObjectSearchUpdate(AJAXMixin, UpdateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        response.render()
        return {
            "inner-fragments": {"#o%d" % self.object.pk: response.content},
        }
    ...
    def post(self, request, *args, **kwargs):
        ...
        if self.fvalid:
            return {
                "inner-fragments": {
                    "#o%d"
                    % self.object.pk: render_to_string(
                        "laboratory/shelfObject.html",
                        {"object": self.object, "laboratory": self.lab},
                        request,
                    )
                },
            }
...
def get_shelfobject_template(request, lab, org, shelfobject):
    return render_to_string(
        template_name="laboratory/components/shelfobject.html",
        context={"shelfobject": shelfobject, "laboratory": lab, "org_pk": org},
        request=request,
    )
...
def get_shelf_list(request):
    ...
    if form.is_valid():
        ...
        transfer_detail = TranferObject.objects.filter(
            ...
            for furniture in furnitures:
                ...
                if len(data) > 0:
                    for shelf in Shelf.objects.filter(pk__in=data):
                        if (
                            unit == shelf.measurement_unit
                            or shelf.measurement_unit is None
                        ):
...
def objects_transfer(request, org_pk, lab_pk, transfer_pk, shelf_pk):
    ...
    if lab_send_obj.quantity >= transfer.quantity:
        lab_received_obj = ShelfObject.objects.filter(
            object_id=obj.id, shelf_id=shelf_pk
        ).first()
        ...
        if lab_received_obj is not None:
            ...
            log_object_change(
                request.user,
                transfer.laboratory_received.pk,
                lab_received_obj,
                old,
            ...
            new_object = ShelfObject.objects.create(
                shelf=shelf,
                object=obj,
            ...
            url = domain + reverse(
                "laboratory:rooms_list", kwargs={"org_pk": org_pk, "lab_pk": lab_pk}
            )
            url = url + "#labroom=%d&furniture=%d&shelf=%d&shelfobject=%d" % (
            ...
            img, file = utils.generate_QR_img_file(
                url, request.user, extension_file=".svg", file_name="qrcode"
            )
        ...
        log_object_change(
            request.user,
            transfer.laboratory_send.pk,
            lab_send_obj,
            old,
            lab_send_obj.quantity,
            "Transferencia de %s al laboratorio %s"
            % (transfer.get_object_detail(), transfer.laboratory_received.name),
            2,
            "Transfer",
            create=False,
            organization=organization,
        ...
        return JsonResponse(
            {
                "status": False,
                "msg": _("The amount sends is more than the laboratory have"),
            }
        )
...
def download_shelfobject_qr(request, org_pk, lab_pk, pk):
    ...
    response["Content-Disposition"] = 'attachment; filename="shelfobject_%s.svg"' % (
        shelfobject.pk
    )
    return response
...
def view_equipment_shelfobject_detail(request, org_pk, lab_pk, pk):
    ...
    context = {
        ...
        "update_maintenance_form": UpdateShelfobjectMaintenanceForm(
            ...
            prefix="update_maintenance",
            org_pk=org_pk,
        ),
        "create_log_form": ShelfobjectLogForm(
            initial={
                "organization": org_pk,
                "created_by": request.user.pk,
                "shelfobject": pk,
