...
@method_decorator(permission_required('laboratory.add_informscheduler'), name='dispatch')
...
@method_decorator(permission_required('laboratory.change_informscheduler'), name='dispatch')
...
@method_decorator(permission_required('laboratory.view_informscheduler'), name='dispatch')
...
def get_inform_index(request, org_pk):
    ...
    schedulers = InformScheduler.objects.filter(organization__pk=org_pk).order_by('active')

    return render(request, 'informs/index.html', context={
        'org_pk': org_pk,
        'labs': labs,
