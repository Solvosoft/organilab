    ...
    if informs:
        ...
        return redirect(
            reverse(
                ...
                kwargs={"lab_pk": kwargs.get("lab_pk"), "org_pk": kwargs.get("org_pk")},
            )
        )
    return redirect(
        reverse(
...
def create_informs(request, *args, **kwargs):
    ...
    organization = get_object_or_404(
        OrganizationStructure.objects.using(settings.READONLY_DATABASE), pk=org
    )
    ...
    if form.is_valid():
        ...
        content = ContentType.objects.get(
            app_label=kwargs.get("content_type"), model=kwargs.get("model")
        )
        inform.content_type = content
        inform.object_id = int(laboratory)
        inform.schema = get_components_url(
            request, inform.custom_form.schema, org, laboratory
        )  # Changes the url of the components to the respective api
    ...
    return render(
        request,
        "laboratory/inform.html",
        context={"laboratory": laboratory, "org_pk": org},
    )
        ...
        return JsonResponse(
            {
                "url": reverse(
                    "laboratory:get_informs",
                    kwargs={"lab_pk": laboratory, "org_pk": org},
                )
            }
        )
