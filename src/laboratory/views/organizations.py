...
class OrganizationSelectableForm(GTForm, forms.Form):
    ...
    filter_organization = TreeNodeChoiceField(
        queryset=organizations, widget=genwidgets.Select, label=_("Filter Organization")
    )
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.profile:
            if self.organization:  # when a organizations is selected
                ...
                if (
                    self.organization in organizations_child
                ):  # user have perm on that organization ?
                ...
                organizations_child = list(
                    OrganizationStructure.os_manager.filter_user(self.user).values_list(
                        "pk", flat=True
                    )
                )
...
class OrganizationActionsFormview(FormView):
    ...
    def clone_organization(self, form):
        ...
        for user in UserOrganization.objects.filter(
            organization=self.org,
