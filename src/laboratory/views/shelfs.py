...
def delete_shelf(request, org_pk, lab_pk, pk, row, col):
    ...
    url = reverse(
        ...
        kwargs={"lab_pk": lab_pk, "pk": pk, "row": row, "col": col, "org_pk": org_pk},
    )
    return {
        "inner-fragments": {
            "#modalclose": """<script>delete_shelf("#shelf_%s", "%s");</script>"""
            % (pk, url)
        },
    }
...
class ShelfForm(forms.ModelForm, GTForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            "type": genwidgets.SelectWithAdd(
                attrs={"add_url": reverse_lazy("laboratory:add_shelf_type_catalog")}
            ),
    ...
    def clean(self):
        ...
        if unit is None and quantity <= 0 and not infinity:
            self.add_error(
                "quantity",
                _(
                    "The quantity need to be greater than 0, when dont select an unit and check the field infinity quantity."
                ),
            )
            self.add_error(
                "measurement_unit",
                _("Need a unit when the dont check the field infinity quantity."),
            )
    ...
    def clean_measurement_unit(self):
        ...
        if unit is None and discard and not infinity:
            self.add_error(
                "measurement_unit",
                _("When a shelf if discard you need to add a measurement unit"),
            )
...
class ShelfUpdateForm(forms.ModelForm, GTForm):
    ...
    def clean_measurement_unit(self):
        ...
        if shelfobjects > 0 and change_unit:
            self.add_error(
                "measurement_unit",
                _(
                    "The shelf have objects need to removed them, before changes the measurement unit"
                ),
            )
        elif unit is None and infinity is False and quantity <= 0:
            self.add_error(
                "quantity",
                _(
                    "The quantity need to be greater than 0, when dont select an unit and check the field infinity quantity."
    ...
    def clean_quantity(self):
        ...
        amount = (
            quantity >= self.instance.get_total_refuse()
        )  # get_total_refuse return the amount that the shelf have about shelfobjects
        if discard:
            if amount and quantity > 0:
                ...
                self.add_error(
                    "quantity",
                    _("The quantity is less than the amount to the sum the objects"),
                )
    ...
    def clean_available_objects_when_limit(self):
        is_limit = self.cleaned_data["limit_only_objects"]
        ...
        if is_limit:
            ...
            if other_objects.exists():
                self.add_error(
                    "available_objects_when_limit",
                    _(
                        "The shelf has other objects besides those selected that should be added or removed from it: "
                    ),
                )

                for obj in other_objects.distinct("object"):
                    self.add_error(
                        "available_objects_when_limit", "- " + obj.object.name
                    )
        return objects
...
class ShelfCreate(AJAXMixin, CreateView):
    ...
    def generate_qr(self):
        ...
        url = url + "#labroom=%d&furniture=%d&shelf=%d" % (
            self.object.furniture.labroom.pk,
            self.object.furniture.pk,
            self.object.pk,
        )
    ...
    def form_valid(self, form):
        ...
        try:
            ...
            logger.error("Error shelf create", exc_info=e)
            return self.form_invalid(form)
        ...
        return {
            "inner-fragments": {"#modalclose": "<script>closeModal();</script>"},
            "append-fragments": {
...
class ShelfEdit(AJAXMixin, UpdateView):
    ...
    def form_valid(self, form):
        ...
        try:
            ...
            logger.error("Invalid shelf edit", exc_info=e)
            return self.form_invalid(form)
