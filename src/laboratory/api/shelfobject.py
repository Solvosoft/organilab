...
from laboratory.shelfobject.serializers import (
    ...
    ShelfObjectLimitsSerializer,
    ShelfObjectStatusSerializer,
    ShelfObjectDeleteSerializer,
    TransferOutShelfObjectSerializer,
    TransferObjectDataTableSerializer,
    TransferInShelfObjectApproveWithContainerSerializer,
    ShelfObjectPk,
    SearchShelfObjectSerializerMany,
    MoveShelfObjectWithContainerSerializer,
    ManageContainerSerializer,
    ValidateShelfInformationPositionSerializer,
    EquimentShelfobjectCharacteristicSerializer,
    MaintenanceSerializer,
    MaintenanceDatatableSerializer,
    UpdateMaintenanceSerializer,
    CreateMaintenanceSerializer,
    ShelfObjectLogDatatableSerializer,
    ValidateShelfObjectLogSerializer,
    ShelfObjectLogSerializer,
    ValidateShelfObjectCalibrateSerializer,
    ShelfObjectCalibrateSerializer,
    ShelfObjectCalibrateDatatableSerializer,
    UpdateShelfObjectCalibrateSerializer,
    ShelfObjectGuaranteeDatatableSerializer,
    ShelfObjectGuaranteeSerializer,
    ValidateShelfObjectTrainingSerializer,
    ShelfObjectTrainingeDatatableSerializer,
    ShelfObjectTrainingSerializer,
    ShelfObjectLogtFilter,
    ShelfObjectCalibrateFilter,
    ShelfObjectTrainingFilter,
    ShelfObjectMaintenanceFilter,
    ValidateShelfObjectGuaranteeSerializer,
...
class ShelfObjectTableViewSet(mixins.ListModelMixin, viewsets.GenericViewSet):
    ...
    def list(self, request, org_pk, lab_pk, **kwargs):
        ...
        validate_serializer = ShelfLabViewSerializer(
            data=request.GET, laboratory=self.laboratory
        )
        ...
        response = {
            "data": data,
            "recordsTotal": ShelfObject.objects.count(),
...
class ShelfObjectCreateMethods:
    ...
    def create_reactive(self, serializer, limits_serializer):
        ...
        container_select_option = serializer.validated_data.pop(
        ...
        container = get_or_create_container_based_on_selected_option(
            container_select_option,
            organization_id,
            laboratory_id,
    ...
    def create_refuse_reactive(
        self, serializer, limits_serializer, equiment_serializer=None
    ):
        """
    ...
    def create_material(self, serializer, limits_serializer):
        ...
        measurement_unit = get_object_or_404(
            Catalog, key="units", description="Unidades"
        )
...
class ShelfObjectViewSet(viewsets.GenericViewSet):
    ...
    def _check_permission_on_laboratory(self, request, org_pk, lab_pk, method_name):
        if request.user.has_perms(
            self.permissions_by_endpoint[method_name]
    ...
    def _get_shelfobject_with_check(self, pk, laboratory_id):
        ...
        if (
            obj.in_where_laboratory is None
            or obj.in_where_laboratory.pk != laboratory_id
        ):
            raise Http404
        return obj
    ...
    def _get_create_shelfobject_serializer(self, request, org_pk, lab_pk):
        ...
        serializer = shelfobject_serializers.ValidateShelfSerializerCreate(
            data=request.data, context={"org_pk": org_pk, "lab_pk": lab_pk}
        )
    ...
    def create_shelfobject(self, request, org_pk, lab_pk, **kwargs):
        ...
        self._check_permission_on_laboratory(
            request, org_pk, lab_pk, "create_shelfobject"
        )
        self.serializer_class = self._get_create_shelfobject_serializer(
            request, org_pk, lab_pk
        )
        ...
        limit_serializer = ShelfObjectLimitsSerializer(
            data=request.data,
            context={
                "type_id": object_type,
        ...
        if serializer.is_valid():
            if limit_serializer.is_valid():
                error, shelfobject = self.serializer_class["method"](
                    serializer, limit_serializer
                )
                if shelfobject:
                    if (
                        not shelfobject.measurement_unit.description == "Unidades"
                        and shelfobject.object.type != "0"
                    ):
                        shelfobject.measurement_unit = (
                            shelfobject.shelf.measurement_unit
                        )
    ...
    def fill_decrease_shelfobject(self, request, org_pk, lab_pk, **kwargs):
        ...
        """
        self._check_permission_on_laboratory(
            request, org_pk, lab_pk, "fill_decrease_shelfobject"
        )
        ...
        return JsonResponse(
            {"detail": _("Shelf object was decrease successfully.")},
            status=status.HTTP_200_OK,
        )
    ...
    def details(self, request, org_pk, lab_pk, pk, **kwargs):
        ...
        serializer = ShelfObjectDetailSerializer(
    ...
    def transfer_out(self, request, org_pk, lab_pk, **kwargs):
        ...
        serializer = self.get_serializer(
            data=request.data, context={"source_laboratory_id": lab_pk}
        )
        errors = {}
        ...
        if serializer.is_valid():
            ...
            if amount_to_transfer <= shelf_object.quantity:
                ...
                errors["amount_to_transfer"] = [
                    _(
                        "This value cannot be greater than the quantity available for the object."
                    )
                ]
        else:
            errors = serializer.errors
        ...
        return JsonResponse(
            {"detail": _("The transfer out was saved successfully.")},
            status=status.HTTP_200_OK,
        )
    ...
    def transfer_available_list(self, request, org_pk, lab_pk, **kwargs):
        ...
        """
        self._check_permission_on_laboratory(
            request, org_pk, lab_pk, "transfer_available_list"
        )
        ...
        self.queryset = TranferObject.objects.filter(
            laboratory_received=lab_pk, status=REQUESTED
        )
    ...
    def transfer_in_deny(self, request, org_pk, lab_pk, **kwargs):
        ...
        """
        self._check_permission_on_laboratory(
            request, org_pk, lab_pk, "transfer_in_deny"
        )
        ...
        if serializer.is_valid():
            ...
            return JsonResponse(
                {"detail": _("The transfer in was denied successfully.")},
                status=status.HTTP_200_OK,
            )
    ...
    def transfer_in_approve(self, request, org_pk, lab_pk, **kwargs):
        ...
        """
        self._check_permission_on_laboratory(
            request, org_pk, lab_pk, "transfer_in_approve"
        )
        ...
        if serializer.is_valid():
            ...
            if previous_quantity == transfer_object.object.quantity:
                ...
                new_shelf_object = move_shelfobject_partial_quantity_to(
                    transfer_object.object,
                    org_pk,
                    lab_pk,
                    serializer.validated_data["shelf"],
        ...
        return JsonResponse(
            {"detail": _("The transfer in was approved successfully.")},
            status=status.HTTP_200_OK,
        )
    ...
    def delete(self, request, org_pk, lab_pk, **kwargs):
        ...
        serializer = ShelfObjectDeleteSerializer(
            data=request.data, context={"laboratory_id": self.laboratory.pk}
        )
        if serializer.is_valid():
    ...
    def create_comments(self, request, org_pk, lab_pk, pk, **kwargs):
        ...
        if serializer_sho.is_valid():
            observation_instance = serializer_sho.save(
                shelf_object=shelf_object, created_by=request.user
            )
            utils.organilab_logentry(
                request.user,
        ...
        return JsonResponse(
            {"detail": _("Observation was created successfully.")},
            status=status.HTTP_200_OK,
        )
    ...
    def update_status(self, request, org_pk, lab_pk, pk, **kwargs):
        ...
        if serializer.is_valid():
            ...
            pre_status = (
                shelfobject.status.description if shelfobject.status else _("No status")
            )
            shelfobject.status = serializer.validated_data["status"]
            shelfobject.save()
            ShelfObjectObservation.objects.create(
                action_taken=_("Status Change of %(pre_status)s of %(description)s")
                % {
    ...
    def move_shelfobject_to_shelf(self, request, org_pk, lab_pk, **kwargs):
        ...
        """
        self._check_permission_on_laboratory(
            request, org_pk, lab_pk, "move_shelfobject_to_shelf"
        )
        ...
        serializer_container = self.serializer_class_container(
            data=request.data,
            context={"laboratory_id": lab_pk, "organization_id": org_pk},
        )
        errors = {}
        ...
        if serializer.is_valid():
            ...
            if shelf_object.object.type == Object.REACTIVE:
                if serializer_container.is_valid():
                    ...
                    amount_converted = get_conversion_from_two_units(
        ...
        return JsonResponse(
            {"detail": _("Object was moved successfully.")}, status=status.HTTP_200_OK
        )
    ...
    def shelf_availability_information(self, request, org_pk, lab_pk, **kwargs):
        ...
        """
        self._check_permission_on_laboratory(
            request, org_pk, lab_pk, "shelf_availability_information"
        )
        self.serializer_class = ValidateShelfInformationPositionSerializer
        serializer = self.serializer_class(
            data=request.query_params, context={"laboratory_id": self.laboratory.pk}
        )
        errors, data = {}, {}
    ...
    def create_status(self, request, org_pk, lab_pk, **kwargs):
        ...
        if serializer.is_valid():
            Catalog.objects.create(
                key="shelfobject_status", description=serializer.data["description"]
            )
            return JsonResponse(
                {"detail": _("The item was created successfully")},
                status=status.HTTP_200_OK,
            )
    ...
    def manage_shelfobject_container(self, request, org_pk, lab_pk, **kwargs):
        self._check_permission_on_laboratory(
            request, org_pk, lab_pk, "manage_shelfobject_container"
        )
        ...
        if serializer.is_valid():
            ...
            new_container_object = get_or_create_container_based_on_selected_option(
                ...
                serializer.validated_data["shelf"],
                request,
                container_for_cloning=serializer.validated_data[
                    "container_for_cloning"
                ],
            ...
            return JsonResponse(
                {"detail": _("The item was updated successfully")},
                status=status.HTTP_200_OK,
            )
    ...
    def edit_equipment_shelfobject(self, request, org_pk, lab_pk, pk, **kwargs):
        ...
        serializer_equipment = EditEquipmentShelfObjectSerializer(
        ...
        if hasattr(shelf_object, "shelfobjectequipmentcharacteristics"):
            serializer_equipment_charac = (
                EditEquimentShelfobjectCharacteristicSerializer(
                    instance=shelf_object.shelfobjectequipmentcharacteristics,
                    data=data,
                    context=context,
                )
            )
...
class SearchLabView(viewsets.GenericViewSet):
        ...
        if shelfobject:
            ...
            if shelfobject_list:
                ...
                result = {
                    ...
                    "shelf": shelf,
                    "furniture": list(
                        shelfobject_list.values_list("shelf__furniture__pk", flat=True)
                    ),
                    "labroom": list(
                        shelfobject_list.values_list(
                            "shelf__furniture__labroom__pk", flat=True
                        )
                    ),
                }
        return result
        ...
        if object_param:
            ...
            shelf = (
                Shelf.objects.filter(
                    shelfobject__object__in=object_list,
                    furniture__labroom__laboratory=lab_pk,
                )
            ...
            result = {
                ...
                "shelf": {
                    "shelf": shelf_pk_list,
                    "furniture": list(
                        shelf.values_list("furniture__pk", flat=True)
                        .distinct()
                        .using(settings.READONLY_DATABASE)
                    ),
                    "labroom": list(
                        shelf.values_list("furniture__labroom", flat=True)
                        .distinct()
                        .using(settings.READONLY_DATABASE)
                    ),
                },
    ...
    def get(self, request, org_pk, lab_pk):
        ...
        search_list, errors = {}, {}
        serializer = SearchShelfObjectSerializerMany(
            data=request.query_params, context={"source_laboratory_id": lab_pk}
        )
...
class ShelfObjectMaintanenceViewset(AuthAllPermBaseObjectManagementShelfObjectBase):
    ...
    def create(self, request, *args, **kwargs):
        ...
        self.lab_pk = kwargs["lab_pk"]
        self.shelfobject = kwargs["shelfobject"]
        serializer = CreateMaintenanceSerializer(
            data=request.data,
            context={
                "org_pk": self.org_pk,
                "lab_pk": self.lab_pk,
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        validate_shelfobject = ValidateShelfobjectEditSerializer(
            data={"shelfobject": self.shelfobject, "organization": self.org_pk},
            context={
                "org_pk": self.org_pk,
                "lab_pk": self.lab_pk,
                "shelfobject": self.shelfobject,
        ...
        return JsonResponse(
            {"detail": "Maintenance do not exists"}, status=status.HTTP_400_BAD_REQUEST
        )
    ...
    def update(self, request, *args, **kwargs):
        ...
        return JsonResponse(
            validate_shelfobject.errors, status=status.HTTP_400_BAD_REQUEST
        )
...
class ShelfObjectLogViewset(AuthAllPermBaseObjectManagementShelfObjectBase):
    ...
    perms = {
        "create": ["laboratory.add_shelfobjectlog", "laboratory.view_shelfobjectlog"],
        "update": [
    ...
    def create(self, request, *args, **kwargs):
        ...
        serializer = ValidateShelfObjectLogSerializer(
            data=request.data,
            context={
                "org_pk": self.org_pk,
                "lab_pk": self.lab_pk,
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        return JsonResponse(
            {"detail": "Log do not exists"}, status=status.HTTP_400_BAD_REQUEST
        )
    ...
    def update(self, request, *args, **kwargs):
        ...
        return JsonResponse(validate_log.errors, status=status.HTTP_400_BAD_REQUEST)
...
class ShelfObjectCalibrateViewset(AuthAllPermBaseObjectManagementShelfObjectBase):
    ...
    def create(self, request, *args, **kwargs):
        ...
        self.lab_pk = kwargs["lab_pk"]
        self.shelfobject = kwargs["shelfobject"]
        serializer = ValidateShelfObjectCalibrateSerializer(
            data=request.data,
            context={
                "org_pk": self.org_pk,
                "lab_pk": self.lab_pk,
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        if validate_calibrate.is_valid(raise_exception=True):
            ShelfObjectObservation.objects.create(
                description=instance.observation,
        ...
        return JsonResponse(
            {"detail": "Calibration do not exists"}, status=status.HTTP_400_BAD_REQUEST
        )
    ...
    def update(self, request, *args, **kwargs):
        ...
        return JsonResponse(
            validate_calibrate.errors, status=status.HTTP_400_BAD_REQUEST
        )
...
class ShelfObjectGuaranteeViewset(AuthAllPermBaseObjectManagementShelfObjectBase):
    ...
    def create(self, request, *args, **kwargs):
        ...
        self.lab_pk = kwargs["lab_pk"]
        self.shelfobject = kwargs["shelfobject"]
        serializer = ValidateShelfObjectGuaranteeSerializer(
            data=request.data,
            context={
                "org_pk": self.org_pk,
                "lab_pk": self.lab_pk,
        ...
        if serializer.is_valid():
            ...
            ShelfObjectObservation.objects.create(
                description=_("New Guarentee from %(initial_date)s to %(final_date)s")
                % {
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        return JsonResponse(
            {"detail": "Guarantee do not exists"}, status=status.HTTP_400_BAD_REQUEST
        )
    ...
    def update(self, request, *args, **kwargs):
        ...
        if validate_shelfobject.is_valid():
            ...
            ShelfObjectObservation.objects.create(
                description=_("Updating guarentee"),
...
class ShelfObjectTrainingViewset(AuthAllPermBaseObjectManagementShelfObjectBase):
    ...
    def create(self, request, *args, **kwargs):
        ...
        self.lab_pk = kwargs["lab_pk"]
        self.shelfobject = kwargs["shelfobject"]
        serializer = ValidateShelfObjectTrainingSerializer(
            data=request.data,
            context={
                "org_pk": self.org_pk,
                "lab_pk": self.lab_pk,
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        return JsonResponse(
            {"detail": "Training do not exists"}, status=status.HTTP_400_BAD_REQUEST
        )
