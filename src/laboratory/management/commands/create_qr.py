...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for shelfobject in ShelfObject.objects.all():
            ...
            if not laboratory:
                print("Laboratory: ", shelfobject.pk, str(shelfobject), shelfobject.shelf.furniture.labroom)
                continue
            org = laboratory.organization
            if not org:
                print("Orga:", shelfobject.pk, str(shelfobject), shelfobject.shelf.furniture)
                continue
            ...
            url = url + "#labroom=%d&furniture=%d&shelf=%d&shelfobject=%d" % \
                  (shelfobject.shelf.furniture.labroom.pk, shelfobject.shelf.furniture.pk,
                   shelfobject.shelf.pk, shelfobject.pk)
