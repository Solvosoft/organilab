...
class Command(BaseCommand):
    ...
    def set_objects(self):
        ...
        for laboratory in laboratories:
            ...
            for unit in units:
                objs = ShelfObject.objects.filter(
                    object__type=0,
                ...
                for obj in objs:
                    ...
                    objects = ObjectLogChange.objects.filter(
                        object=obj, measurement_unit=unit, laboratory=laboratory
                    )
                    shelfobject_total = ShelfObject.objects.filter(
                        object=obj,
                        measurement_unit=unit,
                        in_where_laboratory=laboratory,
    ...
    def set_subject(self):
        ObjectLogChange.objects.filter(
            subject="Ajuste de cantidades en bitacoras", type_action=ADDITION
        ).update(subject=_("Income"))
        ObjectLogChange.objects.filter(
            subject="Ajuste de cantidades en bitacoras", type_action=CHANGE
        ).update(subject=_("Spend"))
