...
class Object(AbstractOrganizationRef):
    ...
    synonym = models.CharField(
        _("Synonym"),
        max_length=255,
        help_text=_("Comma separed name"),
        null=True,
        blank=True,
    )
    type = models.CharField(
        _("Type"), max_length=2, choices=TYPE_CHOICES, default=REACTIVE
    )
...
class SustanceCharacteristics(models.Model):
    ...
    h_code = models.ManyToManyField(
        "sga.DangerIndication", verbose_name=_("Danger Indication"), blank=True
    )
...
class SDSTraceability(BaseCreationObj):
    ...
    source = models.CharField(_('SDS source'), max_length=50, choices=SDS_SOURCE_CHOICES, default='unknown')
    revision_date = models.DateField(_('SDS revision date'), null=True, blank=True)
    download_url = models.URLField(_('Download URL'), max_length=500, blank=True, default='')
    security_sheet = models.FileField(_('Security sheet'), upload_to=upload_files, null=True, blank=True)
    ...
    def __str__(self):
        return f"{self.sustance_characteristics_id} - {self.source} ({self.creation_date})"
...
class EquipmentCharacteristics(models.Model):
    ...
    use_manual = models.FileField(
        upload_to=upload_files, null=True, blank=True, verbose_name=_("Use manual")
    )
    calibration_required = models.BooleanField(
        default=False, verbose_name=_("Is calibration required?")
    )
    operation_voltage = models.CharField(
        max_length=40, null=True, blank=True, verbose_name=_("Operation voltage")
    )
    operation_amperage = models.CharField(
        max_length=40, null=True, blank=True, verbose_name=_("Operation amperage")
    )
    providers = models.ManyToManyField(
        "laboratory.Provider", verbose_name=_("Providers")
    )
    use_specials_conditions = models.TextField(
        null=True, blank=True, verbose_name=_("Use specials conditions")
    )
    generate_pathological_waste = models.BooleanField(
        default=False, verbose_name=_("Generate pathological waste?")
    )
    clean_period_according_to_provider = models.IntegerField(
        null=True, blank=True, verbose_name=_("Clean period according to provider")
    )
    instrumental_family = catalog.GTForeignKey(
        Catalog,
        null=True,
        blank=True,
        key_name="key",
...
class ShelfObject(models.Model):
    ...
    batch = models.CharField(
        max_length=250, default="0", verbose_name=_("Production batch")
    )
    status = catalog.GTForeignKey(
        Catalog,
        null=True,
        on_delete=models.DO_NOTHING,
        verbose_name=_("Status"),
        key_name="key",
        key_value="shelfobject_status",
    )
    ...
    quantity_base_unit = models.FloatField(
        ...
        help_text=_("Quantity in its respective base unit"),
    )
    ...
    limits = models.ForeignKey(
        ShelfObjectLimits, on_delete=models.SET_NULL, null=True, blank=True
    )
    ...
    in_where_laboratory = models.ForeignKey(
        "Laboratory", null=True, blank=False, on_delete=models.CASCADE
    )
    ...
    laboratory_name = models.CharField(
        null=True, blank=True, verbose_name=_("Laboratory name"), max_length=30
    )

    description = models.CharField(
        null=True, blank=True, verbose_name=_("Description"), max_length=256
    )
    creation_date = models.DateTimeField(auto_now_add=True)
    last_update = models.DateTimeField(auto_now=True)
    ...
    def get_measurement_unit_display(self):
        return (
            str(self.measurement_unit) if self.measurement_unit else _("Unknown unit")
        )
...
class ShelfObjectEquipmentCharacteristics(AbstractOrganizationRef):
    ...
    authorized_roles_to_use_equipment = models.ManyToManyField(
        "auth_and_perms.Rol", verbose_name=_("Authorized roles to use equipment")
    )
    equipment_price = models.FloatField(null=True, blank=True, verbose_name=_("Price"))
    purchase_equipment_date = models.DateField(
        null=True, blank=True, verbose_name=_("Purchase Date")
    )
    delivery_equipment_date = models.DateField(
        null=True, blank=True, verbose_name=_("Delivery Date")
    )
    have_guarantee = models.BooleanField(
        default=False, verbose_name=_("Has guarantee?")
    )
    contract_of_maintenance = models.FileField(
        upload_to=upload_files, verbose_name=_("Contract of maintenance")
    )
    notes = models.TextField(null=True, blank=True, verbose_name=_("Note"))
    available_to_use = models.BooleanField(
        default=False, verbose_name=_("Is available to use?")
    )
    first_date_use = models.DateField(
        null=True, blank=True, verbose_name=_("First date use")
    )


class ShelfObjectMaintenance(AbstractOrganizationRef):
    shelfobject = models.ForeignKey(
        ShelfObject, blank=False, null=False, on_delete=models.CASCADE
    )
    maintenance_date = models.DateField(
        null=False, blank=False, verbose_name=_("Maintenance date")
    )
    provider_of_maintenance = models.ForeignKey(
        "laboratory.Provider",
        null=True,
        blank=True,
        on_delete=models.SET_NULL,
    ...
    validator = models.ForeignKey(
        "auth_and_perms.Profile",
        on_delete=models.SET_NULL,
        null=True,
        blank=False,
        verbose_name=_("Validator"),
    )
    maintenance_observation = models.TextField(
...
class ShelfObjectLog(AbstractOrganizationRef):
    shelfobject = models.ForeignKey(
        ShelfObject, null=False, blank=False, on_delete=models.CASCADE
    )
...
class ShelfObjectCalibrate(AbstractOrganizationRef):
    ...
    calibrate_name = models.CharField(
        max_length=100, null=False, blank=True, verbose_name=_("Name of Calibrator")
    )
    validator = models.ForeignKey(
        "auth_and_perms.Profile",
        on_delete=models.SET_NULL,
    ...
    calibration_date = models.DateField(
        null=False, blank=False, verbose_name=_("Calibration date")
    )


class ShelfObjectTraining(AbstractOrganizationRef):
    shelfobject = models.ForeignKey(ShelfObject, null=False, on_delete=models.CASCADE)
    training_initial_date = models.DateField(
        null=False, blank=False, verbose_name=_("Initial date")
    )
    training_final_date = models.DateField(
        null=False, blank=False, verbose_name=_("Final date")
    )
    number_of_hours = models.IntegerField(
        null=False, blank=False, verbose_name=_("Number of hours")
    )
    intern_people_receive_training = models.ManyToManyField(
        "auth_and_perms.Profile", verbose_name=_("Internal participants in training")
    )
    external_people_receive_training = models.TextField(
        blank=True, null=True, verbose_name=_("External people receive training")
    )
    observation = models.TextField(blank=True, null=True, verbose_name=_("Observation"))
    place = models.CharField(
        max_length=100, null=True, blank=True, verbose_name=_("Place")
    )


class ShelfObjectGuarantee(AbstractOrganizationRef):
    shelfobject = models.ForeignKey(ShelfObject, null=False, on_delete=models.CASCADE)
    guarantee_initial_date = models.DateField(
        null=False, blank=False, verbose_name=_("Initial date")
    )
    guarantee_final_date = models.DateField(
        null=False, blank=False, verbose_name=_("Final date")
    )
    contract = models.FileField(
        null=True, blank=True, upload_to=upload_files, verbose_name=_("Contract")
    )


class BaseUnitValues(models.Model):
    measurement_unit_base = catalog.GTForeignKey(
        Catalog,
    ...
    measurement_unit = catalog.GTOneToOneField(
        Catalog,
        related_name="unit",
        on_delete=models.CASCADE,
        verbose_name=_("Unit"),
...
class ShelfObjectObservation(BaseCreationObj):
    action_taken = models.CharField(
        max_length=50, default=_("Object Change"), verbose_name=_("Action Taken")
    )
    description = models.TextField(null=True)
    shelf_object = models.ForeignKey(
        "ShelfObject", on_delete=models.CASCADE, blank=False, null=False
    )


class LaboratoryRoom(BaseCreationObj):
    laboratory = models.ForeignKey(
        "Laboratory", on_delete=models.CASCADE, null=True, blank=False
    )
    name = models.CharField(_("Name"), max_length=255)
...
class Shelf(BaseCreationObj):
    furniture = models.ForeignKey(
        "Furniture", verbose_name=_("Furniture"), on_delete=models.CASCADE
    )
    name = models.CharField(_("Name"), max_length=150, default="nd")
    container_shelf = models.ForeignKey(
        "Shelf",
        null=True,
        blank=True,
    ...
    # C space  D drawer
    type = catalog.GTForeignKey(
        Catalog,
        on_delete=models.DO_NOTHING,
        verbose_name=_("Type"),
        key_name="key",
        key_value="container_type",
    )
    ...
    description = models.TextField(
        null=True, blank=True, default="", verbose_name=_("Description")
    )

    limit_only_objects = models.BooleanField(
        default=False, verbose_name=_("Limit objects to be added")
    )
    available_objects_when_limit = models.ManyToManyField(
        Object,
        related_name="limit_objects",
        verbose_name=_("Only objects allowed in this shelf"),
    )
    infinity_quantity = models.BooleanField(
        default=True, verbose_name=_("Infinite amount")
    )
    ...
    def get_refuse_porcentage(self, include_containers=True, measurement_unit=None):
        ...
        try:
            result = (
                self.get_total_refuse(include_containers, measurement_unit)
                / self.quantity
            ) * 100
    ...
    class Meta:
        permissions = [
            ("can_manage_disposal", "Can manage disposal"),
...
class Furniture(BaseCreationObj):
    labroom = models.ForeignKey(
        "LaboratoryRoom", on_delete=models.CASCADE, verbose_name=_("Labroom")
    )
    name = models.CharField(_("Name"), max_length=255)
    ...
    def get_objects(self):
        return ShelfObject.objects.filter(shelf__furniture=self).order_by(
            "shelf", "-shelf__name"
        )

    def get_limited_shelf_objects(self):
        return ShelfObject.objects.filter(
            shelf__furniture=self, quantity__lte=F("limit_quantity")
        )
...
class OrganizationStructureManager(models.Manager):
    ...
    def organization_tree(
        self, organization, descendants=True, include_self=True, ancestors=True
    ):
        ...
        for org in organizations:
            if descendants:
                pks += list(
                    org.descendants(include_self=include_self).values_list(
                        "pk", flat=True
                    )
                )
            if ancestors:
                pks += list(
                    org.ancestors(include_self=include_self).values_list(
                        "pk", flat=True
                    )
                )
        if pks:
    ...
    def filter_user(
        self, user, descendants=True, include_self=True, ancestors=False, org_pk=None
    ):
        qparams = Q(
...
class OrganizationStructure(TreeNode):
    ...
    def last_child_position(self):
        maxposition = self.descendants().aggregate(maxposition=Max("position"))[
            "maxposition"
        ]
    ...
    def min_child_position(self):
        maxposition = self.descendants().aggregate(maxposition=Min("position"))[
            "maxposition"
        ]
        if maxposition is None:
            maxposition = self.position
...
class UserOrganization(models.Model):
    ...
    type_in_organization = models.IntegerField(
        choices=TYPE_IN_ORG, default=LABORATORY_USER
    )
...
class Laboratory(BaseCreationObj):
    ...
    coordinator = models.CharField(
        _("Coordinator"), default="", max_length=255, blank=True
    )
...
class Provider(BaseCreationObj):
    ...
    phone_number = models.CharField(
        max_length=25, blank=True, default="", verbose_name=_("Phone")
    )
...
class ObjectLogChange(models.Model):
    ...
    measurement_unit = catalog.GTForeignKey(
        Catalog,
        related_name="logmeasurementunit",
        on_delete=models.DO_NOTHING,
        verbose_name=_("Measurement unit"),
        key_name="key",
        key_value="units",
...
class BlockedListNotification(models.Model):
    ...
    object = models.ForeignKey(
        Object, on_delete=models.CASCADE, verbose_name=_("Object")
    )
...
class TranferObject(BaseCreationObj):
    object = models.ForeignKey(
        ShelfObject, on_delete=models.CASCADE, verbose_name=_("Object")
    )
    laboratory_send = models.ForeignKey(
        Laboratory,
        on_delete=models.CASCADE,
        verbose_name=_("Laboratory Send"),
        related_name="lab_send",
    )
    laboratory_received = models.ForeignKey(
        Laboratory,
        on_delete=models.CASCADE,
...
class PrecursorReportValues(models.Model):
    precursor_report = models.ForeignKey(
        "PrecursorReport", on_delete=models.CASCADE, verbose_name=_("Report")
    )
    object = models.ForeignKey(
        Object, on_delete=models.CASCADE, verbose_name=_("Object")
    )
    ...
    previous_balance = models.FloatField(
        default=0.0, verbose_name=_("Previous balance")
    )
    ...
    providers = models.CharField(
        max_length=200, blank=True, verbose_name=_("Providers")
    )
...
class PrecursorReport(models.Model):
    ...
    def get_date_range(self):
        ...
        return _("1 of %s of %d - %d of %s of %d") % (
            month,
            year,
            last_day,
            month,
...
class CommentInform(models.Model):
    created_by = models.ForeignKey(
        User, blank=True, null=True, on_delete=models.CASCADE, verbose_name=_("Creator")
    )
...
class Protocol(BaseCreationObj):
    ...
    file = models.FileField(
        upload_to=upload_files,
        verbose_name=_("Protocol PDF File"),
...
class RegisterUserQR(models.Model):
    ...
    register_user_qr = models.FileField(_("Register user QR"), upload_to=upload_files)
    role = models.ForeignKey(
        "auth_and_perms.Rol", on_delete=models.DO_NOTHING, verbose_name=_("Role")
    )
    ...
    organization_creator = models.ForeignKey(
        OrganizationStructure,
        on_delete=models.CASCADE,
        related_name="organization_creator",
    )
    organization_register = models.ForeignKey(
        OrganizationStructure,
        on_delete=models.CASCADE,
    ...
    code = models.CharField(
        max_length=4, unique=True, null=True, verbose_name=_("Code")
    )
...
class MaterialCapacity(models.Model):
    ...
    capacity_measurement_unit = catalog.GTForeignKey(
        Catalog,
        on_delete=models.DO_NOTHING,
