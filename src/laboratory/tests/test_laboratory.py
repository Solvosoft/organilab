...
class LaboratoryRoomViewTest(BaseLaboratorySetUpTest):
    ...
    def test_get_laboratoryroom_list(self):
        ...
        self.assertTemplateUsed(
            response, template_name="laboratory/laboratoryroom_list.html"
        )

    def test_create_laboratoryroom(self):
        data = {"name": "Sala de investigación"}
        url = reverse(
            "laboratory:rooms_create",
            kwargs={"org_pk": self.org.pk, "lab_pk": self.lab.pk},
        )
        ...
        self.assertIn(
            ...
            list(LaboratoryRoom.objects.values_list("name", flat=True)),
        )

    def test_update_laboratoryroom(self):

        url = reverse(
            "laboratory:rooms_update",
            kwargs={
                "org_pk": self.org.pk,
                "lab_pk": self.lab.pk,
                "pk": self.labroom.pk,
        ...
        data = {"name": "Sala de inventario químico", "created_by": 1}
        ...
        self.assertIn(
            "Sala de inventario químico",
            list(LaboratoryRoom.objects.values_list("name", flat=True)),
        )

    def test_delete_laboratoryroom(self):
        room = LaboratoryRoom.objects.get(
            name="Sala de equipos de protección individual"
        )
        url = reverse(
            "laboratory:rooms_delete",
            kwargs={"org_pk": self.org.pk, "lab_pk": 2, "pk": room.pk},
        )
        response = self.client.post(url, follow=False)
        success_url = reverse(
            "laboratory:rooms_create", kwargs={"org_pk": self.org.pk, "lab_pk": 2}
        )
...
class LaboratoryViewTest(BaseLaboratorySetUpTest):
    ...
    def test_get_laboratory_list(self):
        ...
        self.assertTemplateUsed(
            response, template_name="laboratory/laboratory_list.html"
        )
    ...
    def test_create_laboratory(self):
        ...
        self.assertRedirects(response, success_url)
        self.assertIn(
            "KSA Lab", list(Laboratory.objects.values_list("name", flat=True))
        )

    def test_update_laboratory(self):
        lab = Laboratory.objects.get(name="Lab Ultra")
        url = reverse(
            "laboratory:laboratory_update", kwargs={"org_pk": self.org.pk, "pk": lab.pk}
        )
        response_get = self.client.get(url)
        self.assertEqual(response_get.status_code, 200)
        ...
        self.assertIn(
            "Organización X", list(Laboratory.objects.values_list("name", flat=True))
        )

    def test_check_laboratory_index(self):
        url = reverse(
            "laboratory:labindex", kwargs={"org_pk": self.org.pk, "lab_pk": self.lab.pk}
        )
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
    ...
    def test_laboratory_delete(self):
        ...
        url = reverse(
            "laboratory:laboratory_delete", kwargs={"org_pk": self.org.pk, "pk": lab.pk}
        )
        response = self.client.post(url)
        self.assertEqual(response.status_code, 302)
        self.assertNotIn(
            "DS Cosme", list(Laboratory.objects.values_list("name", flat=True))
        )
