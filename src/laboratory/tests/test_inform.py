...
class InformViewTest(BaseLaboratorySetUpTest):
    ...
    def test_create_inform(self):
        data = {"name": "Uso de instrumentos de laboratorio", "custom_form": 1}
        url = reverse(
            "laboratory:add_informs",
            kwargs={
                "org_pk": self.org.pk,
                "lab_pk": self.lab.pk,
        ...
        self.assertIn(
            ...
            list(Inform.objects.values_list("name", flat=True)),
        )

    def test_delete_inform(self):
        inform = Inform.objects.get(name="Reactivos mensuales despachados")
        url = reverse(
            "laboratory:remove_inform",
            kwargs={"org_pk": self.org.pk, "lab_pk": self.lab.pk, "pk": inform.pk},
        )
        ...
        self.assertRedirects(response, success_url)
        self.assertNotIn(
            "Reactivos mensuales despachados",
            list(Inform.objects.values_list("name", flat=True)),
        )
    ...
    def test_api_informs_detail(self):
        ...
        data = {"period": period.pk}
        response = self.client.get(url, data=data)
        self.assertEqual(response.status_code, 200)
        content_obj = json.loads(response.content)
        self.assertContains(
            response, '<a href=\\"/lab/1/1/informs/complete_inform/1/\\">'
        )
...
class CommentInformViewTest(BaseLaboratorySetUpTest):
    ...
    def test_comment_list_by_inform(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertIn(
            "Describir porque sucedieron los hechos.",
    ...
    def test_add_comment_to_inform(self):
        ...
        data = {
            "inform": inform.pk,
            "comment": "Revisar fecha de ingreso y salida del informe #"
            + str(inform.pk),
        }
        url = reverse("laboratory:api-inform-list", kwargs={"org_pk": self.org.pk})
        response = self.client.post(url, data=data)
    ...
    def test_get_comment(self):
        ...
        url = reverse(
            "laboratory:api-inform-detail",
            kwargs={"pk": comment.pk, "org_pk": self.org.pk},
        )
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
        content_obj = json.loads(response.content)
...
class InformSchedulerViewTest(BaseLaboratorySetUpTest):
    ...
    def test_add_period_scheduler(self):
        url = reverse(
            "laboratory:add_period_scheduler",
            kwargs={
                "org_pk": self.org.pk,
            },
        )
    ...
    def test_edit_period_scheduler(self):
        url = reverse(
            "laboratory:edit_period_scheduler", kwargs={"org_pk": self.org.pk, "pk": 1}
        )
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
    ...
    def test_detail_period_scheduler(self):
        url = reverse(
            ...
            kwargs={"org_pk": self.org.pk, "pk": 1},
        )
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
