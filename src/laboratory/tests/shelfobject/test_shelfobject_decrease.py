...
class ShelfObjectDecreaseViewTest(ShelfObjectSetUp):
    ...
    def test_shelfobject_decrease_case1(self):
        ...
        old_quantity = shelf_object.quantity
        url = reverse("laboratory:api-shelfobject-fill-decrease-shelfobject", kwargs={"org_pk": self.org1.pk, "lab_pk": self.lab1_org1.pk})
        data = {
            "amount": 2,
        ...
        shelf_object = ShelfObject.objects.get(pk=data["shelf_object"])
        new_quantity = shelf_object.quantity
        self.assertEqual(new_quantity, old_quantity - data["amount"])
        self.assertNotEqual(new_quantity, old_quantity)
    ...
    def test_shelfobject_decrease_case2(self):
        ...
        shelf_object = ShelfObject.objects.get(pk=1)
        old_quantity = shelf_object.quantity
        url = reverse("laboratory:api-shelfobject-fill-decrease-shelfobject",
        ...
        shelf_object = ShelfObject.objects.get(pk=data["shelf_object"])
        new_quantity = shelf_object.quantity
        self.assertNotEqual(new_quantity, old_quantity - data["amount"])
