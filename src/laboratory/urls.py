...
from laboratory.views.informs import get_informs, create_informs, complete_inform
...
informs_urls = [
    re_path('informs$', get_informs, name="get_informs"),
    re_path('add_informs/(?P<lab>\d+)/(?P<content_type>\w+)/(?P<model>\w+)$', create_informs, name="add_informs"),
    re_path('complete_inform/(?P<pk>\d+)$', complete_inform, name="complete_inform"),
]
...
] +reports_all_lab+edit_objects+informs_urls
