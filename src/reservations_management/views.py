...
from .functions import add_decrease_stock_task, _allocate_boxes_if_needed, _increase_box_stock, _increase_standard_stock
from .models import Reservations, ReservedProducts, ACCEPTED, DENIED, BORROWED, REQUESTED, RETURNED, CLOSED
    ...
    def post(self, request, *args, **kwargs):
        ...
        if product.initial_date > timezone.now():
            return JsonResponse({'success': False, 'error': str(
                _('The reservation has not started yet. Returns are not allowed before the initial date.')
            )})
        ...
        if product.shelf_object.is_box:
            try:
                ...
                return JsonResponse({'success': False, 'error': str(_('Invalid data.'))})

            if not boxes_to_return:
                return JsonResponse({'success': False, 'error': str(_('Select at least one box to return.'))})

            reserved_map = {b['code']: b['units'] for b in product.reserved_boxes}
            for item in boxes_to_return:
                if item['code'] not in reserved_map:
                    return JsonResponse({'success': False, 'error': str(_('Invalid box code.'))})
                if not isinstance(item['units'], (int, float)) or item['units'] < 0:
                    return JsonResponse({'success': False, 'error': str(
                        _('Units must be greater than 0 for box %(code)s.') % {'code': item['code']}
                    )})
                if item['units'] > reserved_map[item['code']]:
                    return JsonResponse({'success': False, 'error': str(
                        _('Units exceed reserved amount for box %(code)s.') % {'code': item['code']}
                    )})
            ...
            try:
                ...
                return JsonResponse({'success': False, 'error': str(_('Invalid amount.'))})

            if amount < 0 or amount > product.amount_required:
                return JsonResponse({'success': False, 'error': str(
                    _('Amount must be between 0 and %(max)s.') % {'max': product.amount_required}
                )})
        ...
        return JsonResponse({'success': False, 'error': str(_('Could not process the return. Please verify the values.'))})
