...
class ReservationsTest(TestCase):
    ...
    def test_get_update_reservation(self):
        ...
        response = self.client.get(
            reverse("reservations_management:manage_reservation", kwargs=self.url_attr)
        )
        self.assertEqual(response.status_code, 200)
        self.assertTrue(response.context["object"] == reservation)
        self.assertTemplateUsed(
            response, template_name="reservations_management/manage_reservation.html"
        )

    def test_post_update_reservation(self):
        self.url_attr["pk"] = 1
        data = {"status": 3, "comments": "Errors"}
    ...
    def test_getlist_reservation(self):
        ...
        response = self.client.get(
            reverse("reservations_management:reservations_list", kwargs=self.url_attr)
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_get_info_reservation(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertTrue(
            json.loads(response.content)["product_name"] == "Embudo espiga corta 10 cm"
        )
    ...
    def test_validate_reservation(self):
        ...
        response = self.client.get(
            reverse(
                "reservations_management:validate_reservation", kwargs=self.url_attr
            ),
            data=data,
        )
        self.assertEqual(response.status_code, 200)
        self.assertTrue(json.loads(response.content)["is_valid"] is False)

    def test_increasee_stock(self):
        response = self.client.get(
            reverse("reservations_management:increase_stock", kwargs=self.url_attr)
            + "?id=1&amount_to_return=98.5"
        )
        self.assertEqual(response.status_code, 200)
        self.assertTrue(json.loads(response.content)["was_increase"] is True)
