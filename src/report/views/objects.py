...
from django.db.models import Sum, Min, Count, Case, When, F, FloatField, ExpressionWrapper, Cast
from django.db.models.transforms import KeyTextTransform
...
def report_objectlogchange_html(report):
    ...
    record = ObjectChangeLogReportBuilder.objects.filter(
...
def report_reactive_precursor_html(report):
    ...
    columns_fields = columns + [
        ...
        {"name": "quantity_total", "title": _("Quantity total")},
        {"name": "measurement_unit", "title": _("Measurement units")},
        {"name": "molecular_formula", "title": _("Molecular formula")},
        {"name": "cas_id_number", "title": _("CAS id number")},
        {"name": "precursor", "title": _("Is precursor?")},
...
def report_objects_html(report):
    ...
    columns_fields = columns + [
        ...
        {"name": "name", "title": _("Name")},
        {"name": "type", "title": _("Type")},
        {"name": "features", "title": _("Features")},
...
def get_limited_shelf_objects(query):
    for shelf_object in query.filter(
        ...
        current = shelf_object.quantity_box if shelf_object.is_box else shelf_object.quantity
        if (
        ...
        for lab in labs:
            ...
            for shelfobj in shelf_objects:
                data_column = {
                    ...
                    "quantity": shelfobj.quantity_box if shelfobj.is_box else shelfobj.quantity,
...
def get_dataset_report_organization_reactive(report, column_list=None):
    ...
    if "organization" in report.data:
        ...
        objs = (
            ObjectLogChange.objects.filter(**filters, user__in=users)
            .values(
                "object",
                "laboratory__name",
                "object__code",
