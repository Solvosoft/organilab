...
from report.utils import (
    get_pdf_table_content,
    create_notification,
    get_report_name,
    document_status,
    calc_duration,
    check_import_obj,
    get_pdf_log_change_table_content,
)
    ...
    if report.type_report in register.REPORT_FORMS:
        ...
        if report.file_type in register.REPORT_FORMS[report.type_report]:
            ...
            if report.file_type == "pdf":
                if "html" in report_obj:
                    ...
                    if import_html and import_pdf:
                        ...
                        record_total = import_pdf(
                            report, absolute_uri
                        )  # (ABSOLUTE URL MEDIA REQUIRED)
            ...
            description = (
                ...
                + "."
                + report.file_type
                + f" {_('created in')} {t} {tx} {_('with')} {record_total} {_('records')}"
            )
            ...
            if "form" in type_report:
                ...
                if import_form:
                    ...
                    if form.is_valid():
                        ...
                        if import_task:
                            task_celery = import_task.delay(
                                task.pk, request.build_absolute_uri()
                            )
                            task_celery = task_celery.task_id

                            response["result"] = True
                            response.update(
                                {"report": task.pk, "celery_id": task_celery}
                            )
                        else:
                            status_code = 401
                    else:
...
def report_table(request, org_pk, pk):
    ...
    elif task.type_report == "report_regency":
        template_name = "report/regency.html"
