...
from report.api.serializers import ReportDataTableSerializer, \
    ObjectChangeDataTableSerializer, ValidateObjectChangeFilters
...
class ReportDataLogViewSet(mixins.RetrieveModelMixin,viewsets.GenericViewSet):
    authentication_classes = [SessionAuthentication]
    permission_classes = [IsAuthenticated]
    queryset = ObjectChangeLogReportBuilder.objects.all().using(settings.READONLY_DATABASE)
    serializer_class = ObjectChangeDataTableSerializer
    pagination_class = LimitOffsetPagination
    filter_backends = (DjangoFilterBackend, SearchFilter, OrderingFilter)
    search_fields = ['update_time','old_value', 'new_value','diff_value',
                     'user__first_name', 'user__last_name', 'user__username']
    filterset_class = ObjectChangeLogFilterSet
    ordering_fields = ['pk','user','update_time','old_value', 'new_value','diff_value']
