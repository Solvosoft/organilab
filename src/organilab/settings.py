...
if os.getenv("ALLOWED_HOSTS", ""):
    ...
    CSRF_TRUSTED_ORIGINS = [
        CSRF_TRUSTED_SCHEME + "://" + c
        for c in os.getenv("ALLOWED_HOSTS", "").split(",")
    ]
    CORS_ALLOWED_ORIGINS = ["http://localhost:3516"] + [
        CSRF_TRUSTED_SCHEME + "://" + c
        for c in os.getenv("ALLOWED_HOSTS", "").split(",")
    ]
else:
...
DATABASES = {
    ...
    "una": {
        "ENGINE": "django.db.backends.postgresql",
        "NAME": os.getenv("DBNAME_MIGRATIONS", "orga_x"),
        "USER": os.getenv("DBUSER", "organilab_user"),
        "PASSWORD": os.getenv("DBPASSWORD", "0rg4n1l4b"),
        "HOST": os.getenv("DBHOST", "127.0.0.1"),
        "PORT": os.getenv("DBPORT", "5432"),
...
UVICORN_WORKER_CLASS = (
    "djgentelella.firmador_digital.config.asgi_worker.DjgentelellaUvicornWorker"
)
