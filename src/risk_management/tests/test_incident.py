...
class IncidentReportTest(TestCase):
    ...
    def test_get_incident(self):
        ...
        response = self.client.get(reverse('riskmanagement:incident_list', kwargs=self.url_attr))
        self.assertEqual(response.status_code, 200)
        self.assertTrue(len(response.context['object_list'])==3)

    def test_get_q_incident(self):
        response = self.client.get(reverse('riskmanagement:incident_list', kwargs=self.url_attr)+'?q=Algo')
        self.assertEqual(response.status_code, 200)
    ...
    def test_add_incident(self):
        ...
        response = self.client.post(reverse('riskmanagement:incident_create', kwargs=self.url_attr), data=data)
    ...
    def test_update_incident(self):
        ...
        response = self.client.post(reverse('riskmanagement:incident_update', kwargs=self.url_attr), data=data)
    ...
    def test_delete_incident_report(self):
        ...
        response = self.client.post(reverse('riskmanagement:incident_delete',kwargs=self.url_attr))
    ...
    def test_report_incident(self):
        ...
        response = self.client.get(reverse('riskmanagement:incident_report',kwargs=self.url_attr))
