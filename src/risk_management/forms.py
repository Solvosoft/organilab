    ...
    def __init__(self, *args, **kwargs):
        ...
        if queryset.exists() and org_pk:
            ...
            queryset = queryset.filter(
                Q(organization__pk=org_pk) | Q(pk__in=extra_labs)
            ).distinct()

        self.fields["zone_type"].widget.attrs["add_url"] = (
            reverse("riskmanagement:zone_type_add", args=(org_pk,))
            + "?tipo="
            + quote("zone_type")
        )
    ...
    def __init__(self, *args, **kwargs):
        ...
        if org_pk:
            ...
            if instance:
                ...
                self.fields["nearby_buildings"].queryset = Buildings.objects.filter(
                    organization__pk=org_pk
    ...
    class Meta:
        ...
        widgets = {
            "user": AutocompleteSelect(
                "user_organization", attrs={"data-s2filter-org_pk": "#org"}
            ),
            "laboratories": AutocompleteSelectMultiple(
                "regent_laboratories", attrs={"data-s2filter-org_pk": "#org"}
            ),
...
class StructureForm(GTForm, forms.ModelForm):
    ...
    grid_representation = [
        [
            ...
            ["area", "measuerement_unit", "geolocation"],
        ],
    ]
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["geolocation"].widget.attrs["class"] = "form-control"
        self.fields["geolocation"].label = _("Geolocation")
        self.fields["type_structure"].widget.attrs["add_url"] = reverse_lazy(
