...
@method_decorator(
    permission_required("risk_management.view_incidentreport"), name="dispatch"
)
    ...
    def get_queryset(self):
        ...
        if filter_search:
            queryset = queryset.filter(
                Q(short_description__icontains=filter_search)
                | Q(laboratories__name__icontains=filter_search),
...
@method_decorator(
    permission_required("risk_management.add_incidentreport"), name="dispatch"
)
class IncidentReportCreate(djgeneric.CreateView):
    ...
    def get_form_kwargs(self):
        ...
        kwargs["initial"] = {"laboratories": [self.building.laboratories.all()]}
        return kwargs
    ...
    def get_success_url(self, **kwargs):
        ...
        success_url = reverse_lazy(
            "riskmanagement:riskzone_list", kwargs={"org_pk": org_pk}
        )
        return success_url


@method_decorator(
    permission_required("risk_management.change_incidentreport"), name="dispatch"
)
class IncidentReportEdit(djgeneric.UpdateView):
    ...
    def get_form_kwargs(self):
        ...
        kwargs["user"] = self.request.user
        kwargs["org_pk"] = self.org
        kwargs["initial"] = {"laboratories": [self.lab]}
        return kwargs
...
@method_decorator(
    permission_required("risk_management.delete_incidentreport"), name="dispatch"
)
...
def report_incidentreport(request, org_pk, risk_pk, pk):
    ...
    if fileformat in ["xls", "xlsx", "ods"]:
        return django_excel.make_response_from_book_dict(
            make_book_incidentreport(incidentreport),
            fileformat,
            file_name="incident.%s" % (fileformat,),
        )
