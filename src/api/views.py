...
class LaboratoryRoomAPIView(GenericAPIView):
    ...
    def get(self, request, lab_pk, pk=None):
        ...
        if perm:
            ...
            if params:  # it can have filters params
                self.queryset = filters_params_api(
                    self.queryset, params, LaboratoryRoom)
    ...
    def put(self, request, lab_pk, pk):
        ...
        """
        user = request.user
        (lab, perm) = get_valid_lab(
            lab_pk, user, 'laboratory.change_laboratoryroom')
        if perm:
            lab_room = get_object_room(lab, pk)
            if lab_room:
                serializer = LaboratoryRoomSerializer(
                    lab_room, data=request.data)
    ...
    def delete(self, request, lab_pk, pk):
        ...
        user = request.user
        (lab, perm) = get_valid_lab(
            lab_pk, user, 'laboratory.delete_laboratoryroom')
        if perm:
            lab_room = get_object_room(lab, pk)
            if lab_room:
                lab_room.delete()
...
class FurnitureAPIView(GenericAPIView):
    ...
    def get(self, request, lab_pk, pk=None):
        ...
        if perm:
            ...
            if params:  # it can have filters params
                self.queryset = filters_params_api(
                    self.queryset, params, Furniture)
        ...
        if perm:
            ...
            if furniture_shelf:
                serializer = ShelfSerializer(
                    furniture_shelf, data=request.data)
                if serializer.is_valid():
                    serializer.save()
                    return Response(serializer.data)
                else:
...
class ShelfObjectAPIView(GenericAPIView):
    ...
    def get_shelfObjects(self, lab_pk):
        ...
        self.queryset = ShelfObject.objects.filter(
            shelf__furniture__in=furnitures)
    ...
    def get(self, request, lab_pk, pk=None):
        ...
        if perm:
            ...
            if params:  # it can have filters params
                self.queryset = filters_params_api(
                    self.queryset, params, ShelfObject)
